document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.innerWidth <= 768 && typeof html2pdf !== "undefined") {
      ...
      if (!document.getElementById("mobile-pdf-fab")) {
        ...
        fabContainer.style.cssText =
          "position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); z-index: 9999999; display: flex; gap: 10px; background: rgba(0,0,0,0.8); padding: 10px 20px; border-radius: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.3);";

        const btnDownload = document.createElement("button");
        btnDownload.innerHTML =
          '<i class="fa-solid fa-download"></i> Download PDF';
        btnDownload.style.cssText =
          "background: #10b981; color: white; border: none; padding: 10px 15px; border-radius: 20px; font-weight: bold; cursor: pointer; display: flex; align-items: center; gap: 5px;";

        const btnClose = document.createElement("button");
        btnClose.innerHTML = '<i class="fa-solid fa-xmark"></i> Tutup';
        btnClose.style.cssText =
          "background: #ef4444; color: white; border: none; padding: 10px 15px; border-radius: 20px; font-weight: bold; cursor: pointer; display: flex; align-items: center; gap: 5px;";

        btnDownload.onclick = function () {
          let target = null;
          if (document.body.classList.contains("printing-sj"))
            target = document.getElementById("print-sj-container");
          else if (document.body.classList.contains("printing-po"))
            target = document.getElementById("print-area");
          else if (document.body.classList.contains("printing-po-internal"))
            target = document.getElementById("print-container");
          else if (document.body.classList.contains("printing-inv")) {
            if (
              document.getElementById("print-proforma-container") &&
              document
                .getElementById("print-proforma-container")
                .innerHTML.trim() !== ""
            ) {
          ...
          if (target) {
            ...
            target.style.cssText +=
              "; display: block !important; visibility: visible !important; position: relative !important; width: 100% !important; background: white !important; padding: 10px !important;";
  ...
  window.setupDOMPagination = function () {
    ...
    document.addEventListener("click", function (e) {
      const target = e.target.closest(
        'button[id^="pagination-prev-"], button[id^="pagination-next-"]',
      );
      if (!target) return;

      const isPrev = target.id.startsWith("pagination-prev-");
      const tableId = target.id
        .replace("pagination-prev-", "")
      ...
      const rows = Array.from(tbody.querySelectorAll("tr")).filter(
        (tr) =>
          !tr.innerHTML.includes("Memuat data") &&
          !tr.innerHTML.includes("Belum ada") &&
          !tr.innerHTML.includes("Gagal") &&
          !tr.innerHTML.includes("Tidak ada"),
      );
  ...
  window.renderDOMPagination = function (tableId, isPageChange = false) {
    ...
    if (!window.paginationStates[tableId])
      window.paginationStates[tableId] = { page: 1, rowsPerPage: 10 };
    const st = window.paginationStates[tableId];
  ...
  async function sha256(message) {
    try {
      ...
      console.warn(
        "crypto.subtle.digest failed (likely non-HTTPS or localhost). Falling back to plain text for backend to hash.",
      );
      return message;
    }
  }
  ...
  document
    .getElementById("edit-barang-jadi-form")
    ?.addEventListener("submit", async (e) => {
      ...
      if (response.status === "success") {
        ...
        document
          .getElementById("edit-barang-jadi-modal")
  ...
  if (loginForm) {
    ...
    async function checkSession() {
      ...
      if (session) {
        ...
        try {
          const rolePermRes = await window.ERPAPI.request(
          ...
          try {
            ...
            if (rolePermRes.status === "success") {
              ...
              if (
                ...
                JSON.stringify(rolePermRes.data)
              ) {
                user.permissions = rolePermRes.data;
                localStorage.setItem("erp_session", JSON.stringify(user));
                applyUserSession(user);
    ...
    function applyUserSession(user) {
      ...
      navItems.forEach((item) => {
        ...
        if (target === "pengaturan") {
          item.style.display = window.checkPermission("pengaturan", "view")
            ? "flex"
            : "none";
          return;
        }
        ...
        if (item.id === "menu-finance-toggle") {
          const hasFinanceAccess =
            window.checkPermission("finance", "view") ||
            window.checkPermission("invoice", "view") ||
            window.checkPermission("laporan-kas", "view") ||
            window.checkPermission("coa", "view");
          item.style.display = hasFinanceAccess ? "flex" : "none";
          return;
        }

        if (item.id === "menu-produk-toggle") {
          const hasProdukAccess =
            window.checkPermission("purchasing", "view") ||
            window.checkPermission("po-internal", "view") ||
            window.checkPermission("grn", "view") ||
            window.checkPermission("transaksi-gudang", "view") ||
            window.checkPermission("bom", "view") ||
            window.checkPermission("barang-jadi", "view") ||
            window.checkPermission("produksi", "view");
          item.style.display = hasProdukAccess ? "flex" : "none";
          return;
        }

        // Gunakan RBAC helper
        item.style.display = window.checkPermission(target, "view")
          ? "flex"
          : "none";
      });
      ...
      const savedView =
        hashView || localStorage.getItem("erp_active_view") || "dashboard";
      ...
      const targetNav = document.querySelector(
        `.nav-item[data-target="${savedView}"]`,
      );
    ...
    function checkAdminVisibility() {
      ...
      document
        .querySelectorAll(".super-admin-only:not(.nav-item)")
        .forEach((el) => {
    ...
    loginForm?.addEventListener("submit", async (e) => {
      ...
      const res = await window.ERPAPI.request("login", {
        username,
    ...
    const titles = {
      dashboard: {
        ...
        sub: "Selamat datang kembali, Tim Operasional.",
      },
      sales: {
        title: "Penawaran & Sales",
        sub: "Manajemen Penawaran kepada Customer.",
      },
      purchasing: {
        title: "Bahan Baku",
        sub: "Manajemen pengadaan bahan baku.",
      },
      bom: {
        title: "PMO & BOM Sampel",
        sub: "Manajemen Komposisi Material dan Tahapan Produksi.",
      },
      produksi: {
        title: "Produksi & Gudang",
        sub: "Surat Perintah Kerja dan pemotongan stok.",
      },
      finance: {
        title: "Finance & Kasir",
        sub: "Mutasi Kasir dan Petty Cash.",
      },
      "laporan-kas": {
        title: "Laporan Kas",
        sub: "Monitoring aliran dana masuk dan keluar.",
      },
      admin: {
        title: "Manajemen Pengguna",
        sub: "Pengaturan Role Akses Aplikasi.",
      },
      profil: {
        title: "Profil Perusahaan",
        sub: "Informasi dasar identitas perusahaan.",
      },
      coa: {
        title: "Kategori COA & Akuntansi",
        sub: "Master data pos biaya akuntansi.",
      },
      "barang-jadi": {
        title: "Inventori Barang Jadi",
        sub: "Stok produk jadi siap kirim / jual.",
      },
      customer: {
        title: "Master Customer",
        sub: "Database referensi klien / customer perusahaan.",
      },
      "po-customer": {
        title: "PO Customer",
        sub: "Purchase Order dari Customer.",
      },
      "po-internal": {
        title: "Permintaan Belanja",
        sub: "Pengajuan Purchase Order Internal.",
      },
      "surat-jalan": {
        title: "Surat Jalan (DO)",
        sub: "Delivery Order dan Pengiriman.",
      },
      invoice: {
        title: "Invoice Penagihan",
        sub: "Daftar tagihan ke customer.",
      },
      supplier: { title: "Data Supplier", sub: "Database vendor dan pemasok." },
      "menu-produk": {
        title: "Produk & Logistik",
        sub: "Silakan pilih menu manajemen produk dan logistik.",
      },
      "menu-pengaturan": {
        title: "Pengaturan",
        sub: "Pengaturan konfigurasi dan hak akses sistem.",
      },
      "menu-more": { title: "Menu Lainnya", sub: "Pilihan menu tambahan." },
      "menu-lainnya": { title: "Lainnya", sub: "Menu lainnya" },
      "transaksi-gudang": {
        title: "Transaksi Gudang",
        sub: "Manajemen keluar masuk barang di gudang.",
      },
      grn: {
        title: "Good Receipt Note (GRN)",
        sub: "Penerimaan barang dari supplier.",
      },
      rbac: {
        title: "Role Based Access Control",
        sub: "Pengaturan Hak Akses per Role.",
      },
    };
    ...
    function switchView(targetViewId, save = true) {
      ...
      document.querySelectorAll(".nav-item").forEach((nav) => {
        if (
          nav.id === "menu-produk-toggle" ||
          nav.id === "menu-pengaturan-toggle" ||
          nav.id === "menu-finance-toggle"
        )
          return;
        ...
        if (isActive && nav.closest(".submenu")) {
          ...
          if (parentSubmenu.id === "submenu-produk") {
            ...
            document
              .getElementById("menu-pengaturan-toggle")
              .classList.add("open");
          } else if (parentSubmenu.id === "submenu-finance") {
            document
              .getElementById("menu-finance-toggle")
              .classList.add("open");
          }
        }
      });
      ...
      else if (targetViewId === "transaksi-gudang")
    ...
    async function loadDashboardData() {
      try {
        ...
        const [poReq, rmReq, pnwReq, bomReq, spkReq, sjReq, invReq] =
        ...
        if (
          poReq.status === "fulfilled" &&
          poReq.value.status === "success" &&
          poReq.value.data
        ) {
          const spkList =
            spkReq.status === "fulfilled" ? spkReq.value.data || [] : [];
          const sjList =
            sjReq.status === "fulfilled" ? sjReq.value.data || [] : [];
          const invList =
            invReq.status === "fulfilled" ? invReq.value.data || [] : [];

          let unfinishedPOs = [];
          const poOut = poReq.value.data.filter((item) => {
            if (
              String(item.status).toUpperCase() === "BATAL" ||
              String(item.status).toUpperCase() === "DITOLAK"
            )
              return false;
            ...
            const checkMatch = (val) =>
              ...
              ((item.no_penawaran &&
                String(val).trim().toLowerCase() ===
                  String(item.no_penawaran).trim().toLowerCase()) ||
                (item.id_po_customer &&
                  String(val).trim().toLowerCase() ===
                    String(item.id_po_customer).trim().toLowerCase()));
            const relatedSPK = spkList.filter((s) =>
              [s.referensi_po, s.no_penawaran, s.kode_po_customer].some(
                checkMatch,
              ),
            );
            const relatedSJ = sjList.filter((s) =>
              [s.no_penawaran, s.id_po_customer, s.referensi_penawaran].some(
                checkMatch,
              ),
            );
            const relatedInv = invList.filter((s) => {
              if (
                [
                  s.no_penawaran,
                  s.id_po_customer,
                  s.referensi_penawaran,
                  s.referensi_po,
                ].some(checkMatch)
              )
                return true;
              if (s.no_sj) {
                const invSjs = String(s.no_sj)
                  .split(",")
                  .map((x) => x.trim().toLowerCase());
                return relatedSJ.some(
                  (sj) =>
                    (sj.no_sj &&
                      invSjs.includes(String(sj.no_sj).trim().toLowerCase())) ||
                    (sj.no_surat_jalan &&
                      invSjs.includes(
                        String(sj.no_surat_jalan).trim().toLowerCase(),
                      )),
                );
              }
              return false;
          ...
          const activityTbody = document.getElementById(
        ...
        if (
          rmReq.status === "fulfilled" &&
          rmReq.value.status === "success" &&
          rmReq.value.data
        ) {
          const rmKritis = rmReq.value.data.filter(
            (s) => parseFloat(s.stok) <= 10,
          ).length; // Stok <= 10 dianggap kritis
          const rmEl = document.getElementById("dashboard-rm");
          if (rmEl) rmEl.textContent = rmKritis;
        }

        if (
          pnwReq.status === "fulfilled" &&
          pnwReq.value.status === "success" &&
          pnwReq.value.data
        ) {
          const invUnpaid = pnwReq.value.data.filter(
            (p) => String(p.status).toUpperCase() === "PENAWARAN",
          ).length;
        ...
        if (
          bomReq.status === "fulfilled" &&
          bomReq.value.status === "success" &&
          bomReq.value.data
        ) {
    ...
    navItems.forEach((item) => {
      item.addEventListener("click", () => {
        ...
        if (item.id === "menu-pengaturan-toggle") {
          item.classList.toggle("open");
          document
            .getElementById("submenu-pengaturan")
    ...
    if (
      ...
      window.Capacitor.Plugins.App.addListener("backButton", () => {
        ...
        const currentView =
          localStorage.getItem("erp_active_view") || "dashboard";
    ...
    document
      .getElementById("btn-my-profile-mobile")
      ?.addEventListener("click", handleMyProfile);
    ...
    function updateGlobalFAB(viewId) {
      ...
      const fabMappings = {
        ...
        "po-customer": [
          {
            id: "btn-add-po-customer",
            label: "Buat PO dari Penawaran",
            icon: "fa-plus",
            color: "var(--secondary)",
          },
        ],
        "surat-jalan": [
          {
            id: "btn-add-surat-jalan",
            label: "Buat Surat Jalan Manual",
            icon: "fa-plus",
            color: "var(--accent)",
          },
        ],
      ...
      if (
        ...
        const canAdd =
          typeof window.checkRBACAction === "function"
            ? window.checkRBACAction("can_add", viewId)
            : true;
        if (!canAdd) {
          // If they can't add, clear all actions that start with btn-add
          actions = actions
            ? actions.filter((a) => !a.id.startsWith("btn-add"))
            : [];
        }
      }
    ...
    document
      .getElementById("fab-backdrop")
      ?.addEventListener("click", closeFAB);
    ...
    async function loadPurchasingData(isBackgroundSync = false) {
      ...
      if (!isBackgroundSync) {
        if (
          !tbody.innerHTML.trim() ||
          tbody.innerHTML.includes("Tidak ada") ||
          tbody.innerHTML.includes("Gagal") ||
          tbody.innerHTML.includes("Memuat data")
        ) {
          tbody.innerHTML =
      ...
      if (response.status === "success" && response.data) {
        ...
        if (fLokasi) {
          ...
          const uniqueLokasi = [
            ...new Set(
              response.data.map((item) => item.lokasi).filter(Boolean),
            ),
          ];
        ...
        if (satuanList) {
          const uniqueSatuan = [
            ...new Set(
              response.data.map((item) => item.satuan).filter(Boolean),
            ),
          ];
          uniqueSatuan.forEach((sat) => {
            // Hanya tambahkan jika belum ada di opsi default
            if (
              ![...satuanList.options].some(
                (opt) => opt.value.toLowerCase() === sat.toLowerCase(),
              )
            ) {
              const option = document.createElement("option");
              option.value = sat;
        ...
        tbody.innerHTML =
          '<tr><td colspan="5" style="text-align: center; color: var(--danger);">Gagal memuat data / data kosong</td></tr>';
      }
    }

    // --- Event Listeners for Bahan Baku Search, Filter, Pagination ---
    document
      .getElementById("search-bahan-baku")
    ...
    document
      .getElementById("btn-prev-bahan-baku")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btn-next-bahan-baku")
      ?.addEventListener("click", () => {
        bahanBakuCurrentPage++;
        renderPurchasingTable();
      });
    ...
    function renderPurchasingTable() {
      ...
      let filteredData = bahanBakuData.filter((item) => {
        ...
        const matchesGudang =
          gudangFilter === "" || item.lokasi === gudangFilter;
      ...
      const totalPages = Math.max(
        1,
        Math.ceil(totalRows / bahanBakuRowsPerPage),
      );
    ...
    function openDataModal(title, data = null) {
      ...
      document.getElementById("f_kode").value = data
        ? data.kode
      ...
      document.getElementById("f_stok").value =
        data && data.stok ? window.formatRibuan(data.stok) : "";
      document.getElementById("f_satuan").value =
        data && data.satuan ? data.satuan : "";
      ...
      document.getElementById("f_harga").value =
        data && data.harga ? window.formatRibuan(data.harga) : "";
      document.getElementById("f_spesifikasi").value =
        data && data.spesifikasi && data.spesifikasi !== "undefined"
          ? data.spesifikasi
          : "";
    ...
    dataForm?.addEventListener("submit", (e) => {
      ...
      const payload = {
        ...
        harga:
          parseInt(
            String(document.getElementById("f_harga").value).replace(/\D/g, ""),
          ) || 0,
      ...
      const existingIdx = bahanBakuData.findIndex(
        (s) => s.kode === payload.kode,
      );
      if (existingIdx !== -1) {
        bahanBakuData[existingIdx] = {
          ...bahanBakuData[existingIdx],
          ...payload,
        };
      } else {
    ...
    document
      .getElementById("btn-import-stock")
      ?.addEventListener("click", () => {
        document.getElementById("file-import-stock").click();
      });

    document
      .getElementById("btn-export-stock")
      ?.addEventListener("click", () => {
        ...
        try {
          ...
          XLSX.writeFile(
            wb,
            `Data_Bahan_Baku_${new Date().toISOString().slice(0, 10)}.xlsx`,
          );
    ...
    document
      .getElementById("file-import-stock")
        ...
        reader.onload = async (event) => {
          try {
                  ...
                  for (let key in row) {
                    ...
                    const matchedKey = keys.find((k) => {
                      if (
                        cleanKey === k ||
                        cleanKey === k + "material" ||
                        (k === "stok" && cleanKey === "stoksaatini") ||
                        cleanKey.startsWith(k)
                      ) {
                        if (k === "satuan" && cleanKey.includes("harga"))
                          return false;
                        return true;
                      }
                      return false;
            ...
            if (items.length > 0) {
              const session = localStorage.getItem("erp_session");
              const user = session ? JSON.parse(session) : {};
              const res = await window.ERPAPI.request("import_stock", {
                data: items,
                username: user.username,
                user_nama: user.nama,
              });
              ...
              window.showToast?.(
                "Tidak ada data valid yang bisa diimpor. Pastikan ada kolom Kode dan Nama Material.",
                "warning",
              );
            }
          } catch (err) {
    ...
    async function loadPOInternalData(isBackgroundSync = false) {
      ...
      if (!isBackgroundSync) {
        if (
          ...
          tbody.innerHTML =
            '<tr><td colspan="7" style="text-align: center;"><i class="fa-solid fa-spinner fa-spin"></i> Memuat data...</td></tr>';
        }
      }
      ...
      if (response.status === "success" && response.data) {
        ...
        if (response.data.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="7" style="text-align: center; color: var(--text-muted);">Belum ada pengajuan belanja.</td></tr>';
          return;
        }
        ...
        sorted.forEach((item) => {
          ...
          let badgeIcon = "⏳";
          if (item.status === "Disetujui (Sedang Dibelikan)") {
            badgeClass = "badge-success";
            badgeIcon = "✅";
          } else if (item.status === "Selesai (Barang Diterima)") {
            badgeClass = "badge-success";
            badgeIcon = "📦";
          } else if (item.status === "Ditolak") {
            badgeClass = "badge-danger";
            badgeIcon = "❌";
          } else if (item.status === "Parsial") {
          ...
          if (
            (item.status === "Disetujui (Sedang Dibelikan)" ||
              item.status === "Parsial") &&
            isPurchasing
          ) {
        ...
        tbody.querySelectorAll(".btn-detail-po").forEach((btn) => {
          btn.addEventListener("click", () =>
            openPODetail(btn.getAttribute("data-no"), response.data),
          );
        });
    ...
    function openGRNModal(item) {
      ...
      try {
        items =
          typeof item.items_parsed === "object"
            ? item.items_parsed
    ...
    function printPOInternal(item) {
      ...
      try {
        info =
          typeof item.info_tambahan === "string"
            ? JSON.parse(item.info_tambahan)
            : item.info_tambahan || {};
      } catch (e) {}

      document.getElementById("print_po_company_name").textContent =
        (window.cachedSettings || {})["NAMA_PERUSAHAAN"] || "PT. ORION KARYA SEJAHTERA";
      document.getElementById("print_po_company_address").textContent =
        (window.cachedSettings || {})["ALAMAT"] || "Alamat Perusahaan";
      document.getElementById("print_po_company_phone").textContent =
        (window.cachedSettings || {})["NO_TELP"] || "No. Telp";

      document.getElementById("print_po_no").textContent = item.no_po || "-";
      document.getElementById("print_po_date").textContent = item.tanggal
        ? item.tanggal.split(" ")[0]
        : "-";

      document.getElementById("print_po_to").textContent = info.po_to || "-";
      document.getElementById("print_po_attn").textContent =
        info.po_attn || "-";
      ...
      const diskon = parseFloat(info.po_diskon) || 0;
      const ppnPercent = parseFloat(info.po_ppn) || 0;
      const biayaLain = parseFloat(info.po_biaya_lain) || 0;

      const dpp = total - diskon;
      const ppnAmount = dpp * (ppnPercent / 100);
      const subtotalEstimasi = dpp + ppnAmount + biayaLain;

      document.getElementById("print_po_total").textContent =
        "Rp " + total.toLocaleString("id-ID");
      document.getElementById("print_po_discount").textContent =
        "Rp " + diskon.toLocaleString("id-ID");
      document.getElementById("print_po_dpp").textContent =
        "Rp " + Math.round(dpp).toLocaleString("id-ID");

      const ppnLabelEl = document.getElementById("print_po_ppn_label");
      if (ppnLabelEl) ppnLabelEl.textContent = ppnPercent;

      document.getElementById("print_po_ppn").textContent =
        "Rp " + Math.round(ppnAmount).toLocaleString("id-ID");
      document.getElementById("print_po_biaya").textContent =
        "Rp " + Math.round(biayaLain).toLocaleString("id-ID");
      document.getElementById("print_po_subtotal").textContent =
        "Rp " + Math.round(subtotalEstimasi).toLocaleString("id-ID");

      document.getElementById("print_po_req_date").textContent =
        info.req_date || "-";
      document.getElementById("print_po_payment").textContent =
        info.payment_term || "-";
      document.getElementById("print_po_sign_req").textContent =
        item.pemohon || "-";
      document.getElementById("print_po_sign_prep").textContent =
        item.pemohon || "-";
      document.getElementById("print_po_ack2").textContent =
        item.disetujui_oleh || "-";
      document.getElementById("print_po_remarks").textContent =
        info.catatan || "-";
    ...
    async function deletePOInternal(noPO, trElement = null) {
      ...
      if (typeof showToast !== "undefined")
        showToast("Menghapus PO Internal...", "info", 2000);

      window.ERPAPI.request("delete_po_internal", { no_po: noPO }).then(
        (res) => {
          if (res.status === "success") {
    ...
    function openPODetail(noPO, allData) {
      ...
      let infoTambahan = {};
      try {
        infoTambahan = item.info_tambahan ? JSON.parse(item.info_tambahan) : {};
      } catch (e) {}
      ...
      const isAdmin = [
        ...
        "Management",
        "Direktur",
      ].some((r) => (user.role || "").toLowerCase().includes(r.toLowerCase()));
      const isAtasan =
        isAdmin ||
        (user.role || "").toLowerCase().includes("atasan") ||
      ...
      document.getElementById("po-detail-content").innerHTML = `
            ...
            <table style="width:100%; border-collapse:collapse; font-size:0.85rem;">
                ...
                <tfoot>
                    <tr style="border-top:1px dashed var(--glass-border);">
                        <td colspan="5" style="padding:0.5rem 0.75rem; text-align:right; color:var(--text-muted);">Diskon</td>
                        <td style="padding:0.5rem 0.75rem; text-align:right; font-weight:600;">Rp ${(parseFloat(infoTambahan.po_diskon) || 0).toLocaleString("id-ID")}</td>
                    </tr>
                    <tr>
                        <td colspan="5" style="padding:0.5rem 0.75rem; text-align:right; color:var(--text-muted);">DPP</td>
                        <td style="padding:0.5rem 0.75rem; text-align:right; font-weight:600;">Rp ${Math.round((parseInt(item.total_estimasi || 0) || 0) - (parseFloat(infoTambahan.po_diskon) || 0) - (parseFloat(infoTambahan.po_biaya_lain) || 0) - ((parseInt(item.total_estimasi || 0) || 0) - (parseFloat(infoTambahan.po_diskon) || 0) - (parseFloat(infoTambahan.po_biaya_lain) || 0)) * ((parseFloat(infoTambahan.po_ppn) || 0) / (100 + (parseFloat(infoTambahan.po_ppn) || 0)))).toLocaleString("id-ID")}</td>
                    </tr>
                    <tr>
                        <td colspan="5" style="padding:0.5rem 0.75rem; text-align:right; color:var(--text-muted);">PPN ${parseFloat(infoTambahan.po_ppn) || 0}%</td>
                        <td style="padding:0.5rem 0.75rem; text-align:right; font-weight:600;">Rp ${Math.round(((parseInt(item.total_estimasi || 0) || 0) - (parseFloat(infoTambahan.po_diskon) || 0) - (parseFloat(infoTambahan.po_biaya_lain) || 0)) * ((parseFloat(infoTambahan.po_ppn) || 0) / (100 + (parseFloat(infoTambahan.po_ppn) || 0)))).toLocaleString("id-ID")}</td>
                    </tr>
                    <tr>
                        <td colspan="5" style="padding:0.5rem 0.75rem; text-align:right; color:var(--text-muted);">Biaya Lainnya</td>
                        <td style="padding:0.5rem 0.75rem; text-align:right; font-weight:600;">Rp ${(parseFloat(infoTambahan.po_biaya_lain) || 0).toLocaleString("id-ID")}</td>
                    </tr>
      ...
      contentEl
        .querySelector(".btn-edit-po-internal")
        ?.addEventListener("click", (e) => {
          const itemStr = e.currentTarget.getAttribute("data-item");
          if (itemStr) {
            document
              .getElementById("po-detail-modal")
              .classList.remove("active");
            openEditPOInternal(JSON.parse(itemStr));
          }
        });
      contentEl
        .querySelector(".btn-approve-po-action")
        ?.addEventListener("click", () => {
          document.getElementById("po-detail-modal").classList.remove("active");
          updatePOStatusAction(item.no_po, "Disetujui (Sedang Dibelikan)");
        });
      contentEl
        .querySelector(".btn-reject-po-action")
        ?.addEventListener("click", () => {
          document.getElementById("po-detail-modal").classList.remove("active");
      ...
      function openEditPOInternal(item) {
        ...
        document.getElementById("po_to").value = infoTambahan.po_to || "";
        document.getElementById("po_attn").value = infoTambahan.po_attn || "";
        document.getElementById("po_alamat").value =
        ...
        document.getElementById("po_diskon").value =
          infoTambahan.po_diskon || "";
        document.getElementById("po_ppn").value = infoTambahan.po_ppn || "";
        document.getElementById("po_biaya_lain").value =
          infoTambahan.po_biaya_lain || "";
      ...
      async function updatePOStatusAction(noPO, status) {
        ...
        const btn = document.querySelector(
          `[data-no="${noPO}"].btn-approve-po, [data-no="${noPO}"].btn-selesai-po, [data-no="${noPO}"].btn-reject-po`,
        );
        if (btn) {
          btn.disabled = true;
      ...
      function calculatePOTotal() {
        ...
        document.querySelectorAll(".po-item-row").forEach((row) => {
          const harga =
            parseFloat(
              String(row.querySelector(".po-harga-aktual")?.value || "0")
                .replace(/\./g, "")
                .replace(",", "."),
            ) || 0;
          const qty =
            parseFloat(row.querySelector(".po-qty")?.value || "0") || 0;
        ...
        const totalItemDisplay = document.getElementById(
          "po_total_item_display",
        );
        if (totalItemDisplay)
          totalItemDisplay.textContent =
            "Rp " + Math.round(total).toLocaleString("id-ID");

        const diskonEl = document.getElementById("po_diskon");
        const ppnEl = document.getElementById("po_ppn");
        const biayaLainEl = document.getElementById("po_biaya_lain");

        const diskon =
          parseFloat(
            String(diskonEl?.value || "0")
              .replace(/[^0-9,-]+/g, "")
              .replace(",", "."),
          ) || 0;
        const ppn = parseFloat(ppnEl?.value || "0") || 0;
        const biayaLain =
          parseFloat(
            String(biayaLainEl?.value || "0")
              .replace(/[^0-9,-]+/g, "")
              .replace(",", "."),
          ) || 0;

        const dpp = total - diskon;
        const ppnAmount = dpp * (ppn / 100);
        const totalEstimasi = dpp + ppnAmount + biayaLain;

        const dppDisplay = document.getElementById("po_dpp_display");
        if (dppDisplay)
          dppDisplay.textContent =
            "Rp " + Math.round(dpp).toLocaleString("id-ID");

        const totalDisplay = document.getElementById("po-total-display");
        if (totalDisplay)
          totalDisplay.textContent =
            "Rp " + Math.round(totalEstimasi).toLocaleString("id-ID");
      }

      document
        .getElementById("po_diskon")
        ?.addEventListener("input", calculatePOTotal);
      document
        .getElementById("po_ppn")
        ?.addEventListener("input", calculatePOTotal);
      document
        .getElementById("po_biaya_lain")
        ?.addEventListener("input", calculatePOTotal);
      ...
      function addPOItemRow(
        ...
        tr.querySelector(".po-nama").addEventListener("input", (e) => {
          ...
          if (match) {
            const hargaFormatted = parseInt(match.harga || 0).toLocaleString(
              "id-ID",
            );
        ...
        tr.querySelector(".po-harga").addEventListener(
          "input",
          calculatePOTotal,
        );
        tr.querySelector(".po-harga-aktual").addEventListener(
          "input",
          calculatePOTotal,
        );
        ...
        tr.querySelector(".po-remove-item").addEventListener("click", () => {
          tr.remove();
          calculatePOTotal();
          if (document.querySelectorAll(".po-item-row").length === 0)
            addPOItemRow();
        });
      ...
      document
        .getElementById("btn-add-po-item")
        ?.addEventListener("click", () => addPOItemRow());

      document
        .getElementById("btn-add-po-internal")
        ?.addEventListener("click", () => {
          if (supplierData.length === 0) loadSupplierData(true);
          const session = localStorage.getItem("erp_session");
          const user = session ? JSON.parse(session) : {};
          document.getElementById("po-info-pemohon").textContent =
            user.nama || user.username || "System";
          document.getElementById("po-info-tanggal").textContent =
          ...
          if (refSelect) {
            const populateRef = (data) => {
              refSelect.innerHTML =
                '<option value="">-- Pilih PO Customer --</option>' +
                data
                  .map(
                    (po) =>
                      `<option value="${po.id_po_customer}">${po.id_po_customer} - ${po.nama_customer || "-"}</option>`,
                  )
                  .join("");
            };
          ...
          poForm.reset();
          document.getElementById("po_internal_id").value = "";
      ...
      document
        .getElementById("btn-close-po-modal")
        ?.addEventListener("click", () => poModal.classList.remove("active"));
      document
        .getElementById("btn-close-po-modal-footer")
        ?.addEventListener("click", () => poModal.classList.remove("active"));
      document
        .getElementById("btn-close-po-detail")
        ?.addEventListener("click", () =>
          document.getElementById("po-detail-modal").classList.remove("active"),
        );
      ...
      document.getElementById("po_to")?.addEventListener("input", function () {
        ...
        const sup = supplierData.find((s) => {
          const displayStr = s.id_supplier
            ? `${s.id_supplier} - ${s.nama_supplier}`
            : s.nama_supplier;
          return (
            displayStr.trim().toLowerCase() === val ||
            String(s.nama_supplier || "")
              .trim()
              .toLowerCase() === val
          );
        });
        if (sup) {
          ...
          const kontak =
            sup.kontak___telepon || sup["kontak_/_telepon"] || sup.kontak || "";
      ...
      poForm?.addEventListener("submit", async (e) => {
        ...
        document.querySelectorAll(".po-item-row").forEach((row) => {
          ...
          const harga =
            parseFloat(
              String(row.querySelector(".po-harga")?.value || "0")
                .replace(/\./g, "")
                .replace(",", "."),
            ) || 0;
          const harga_aktual =
        ...
        const po_ppn = parseFloat(ppnEl?.value || "0") || 0;
        const po_biaya_lain =
          parseFloat(
            String(biayaLainEl?.value || "0")
              .replace(/[^0-9,-]+/g, "")
        ...
        items.forEach((it) => {
          totalBarang +=
            (it.harga_aktual > 0 ? it.harga_aktual : it.harga) * it.qty;
        });
        const dpp = totalBarang - po_diskon;
        const po_total_estimasi = dpp + (dpp * po_ppn) / 100 + po_biaya_lain;
        ...
        if (typeof showToast !== "undefined")
          showToast("Sinkronisasi PO Internal ke server...", "info", 3000);
        ...
        const poInternalId = document.getElementById("po_internal_id")?.value;
        if (poInternalId) payload.no_po = poInternalId;

        const actionName = poInternalId
          ? "update_po_internal"
          : "create_po_internal";

        window.ERPAPI.request(actionName, payload).then((res) => {
          if (btnSubmit) {
            btnSubmit.innerHTML = "Kirim Pengajuan PO Internal";
            btnSubmit.disabled = false;
          }
      ...
      async function loadBarangJadiData(isBackgroundSync = false) {
        ...
        if (response.status === "success" && response.data) {
          ...
          if (response.data.length === 0) {
            tbody.innerHTML =
              '<tr><td colspan="5" style="text-align: center;">Belum ada data barang jadi.</td></tr>';
            return;
          }
            ...
            if (isAdmin) {
              ...
              if (btnEdit) {
                btnEdit.addEventListener("click", (e) => {
                  e.stopPropagation();
                  document.getElementById("edit_bj_kode").value =
                    btnEdit.getAttribute("data-kode");
                  document.getElementById("edit_bj_nama").value =
                    btnEdit.getAttribute("data-nama");
                  document.getElementById("edit_bj_stok").value =
                    btnEdit.getAttribute("data-stok");
                  document.getElementById("edit_bj_lokasi").value =
                    btnEdit.getAttribute("data-lokasi");
                  document
                    .getElementById("edit-barang-jadi-modal")
              ...
              if (btnReset) {
                btnReset.addEventListener("click", async (e) => {
                  ...
                  if (typeof showToast !== "undefined")
                    showToast("Mereset stok Barang Jadi...", "info");
                  window.ERPAPI.request("edit_barang_jadi", {
                    kode_barang: kode,
                    stok: 0,
                  }).then((res) => {
                    if (res.status === "success") {
                      if (typeof showToast !== "undefined")
                        showToast(`✅ Stok ${nama} direset ke 0`, "success");
                      loadBarangJadiData(true);
                    } else {
                      if (typeof showToast !== "undefined")
                        showToast("❌ Gagal mereset stok", "error");
                      loadBarangJadiData(true);
                    }
                  });
                });
      ...
      window.createDPInvoice = function (poId) {
        const item = window.poCustomerData.find(
          (p) => p.id_po_customer === poId,
        );
        if (!item) return;
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            document.getElementById("inv_tanggal").value = new Date(
              today.getTime() - offset,
            )
              .toISOString()
              .split("T")[0];
            document.getElementById("inv_jatuh_tempo").value = new Date(
              today.getTime() - offset + 7 * 24 * 60 * 60 * 1000,
            )
              .toISOString()
              .split("T")[0];
            ...
            document.getElementById("inv_customer").value =
              item.nama_customer || item.customer || "";

            const tbody = document.getElementById("inv-items-tbody");
            if (tbody) {
              tbody.innerHTML = "";
              const poTotal = window.parseRupiah(item.total_harga || 0);
              const percentage = poTotal
                ? Math.round((dpVal / poTotal) * 100)
                : "";
              window.dpDescTemp =
                `Tagihan Down Payment (DP) ${percentage ? percentage + "%" : ""} untuk PO ${poId}`.trim();

              let itemNames = "";
              try {
                const parsedItems =
                  typeof item.item_po === "string"
                    ? JSON.parse(item.item_po)
                    : item.item_po || [];
                itemNames = parsedItems
                  .map((it) => {
                    let name = it.nama || it.part_name || "";
                    let qty = it.qty || it.jumlah || "";
                    let numQty = String(qty)
                      .replace(/[^0-9.]/g, "")
                      .trim();
            ...
            if (document.getElementById("inv_ppn"))
              document.getElementById("inv_ppn").value = "0";
            if (document.getElementById("inv_catatan")) {
              const poTotal = window.parseRupiah(item.total_harga || 0);
              const sisa = poTotal - dpVal;
              document.getElementById("inv_catatan").value =
                `${window.dpDescTemp}. Sisa tagihan pelunasan: ${window.formatRupiah(sisa)}`;
            }
            ...
            const tabInvoice = document.querySelector(
              '.menu-item[data-tab="invoice"]',
            );
      ...
      async function loadPOCustomerData(isBackgroundSync = false) {
        ...
        if (resPoc.status === "success" && resPoc.data) {
          ...
          if (window.poCustomerData.length === 0) {
            tbody.innerHTML =
              '<tr><td colspan="7" style="text-align: center;">Tidak ada data PO Customer.</td></tr>';
            return;
          }
          window.poCustomerData.forEach((item) => {
            ...
            if (totalTerbayar >= numericTotalHarga && numericTotalHarga > 0) {
              ...
              progress = 0;
              progressLabel =
                totalTerbayar > 0 ? "DP Diterima (Tunggu SPK)" : "Menunggu SPK";
      ...
      document
        .getElementById("btn-export-poc-excel")
        ?.addEventListener("click", async () => {
          ...
          const uniqueCustomers = [
            ...new Set(
              window.poCustomerData
                .map((item) => item.nama_customer)
                .filter(Boolean),
            ),
          ].sort();
          let customerOptions =
            '<option value="">-- Semua Toko / Customer --</option>';
            ...
            try {
              ...
              if (filters.customer) {
                filteredData = filteredData.filter(
                  (item) => item.nama_customer === filters.customer,
                );
              }
              if (filters.status) {
                filteredData = filteredData.filter(
              ...
              if (filteredData.length === 0) {
                window.showToast?.(
                  "Tidak ada data yang sesuai dengan filter tersebut.",
                  "warning",
                );
                return;
              }
              ...
              const exportData = filteredData.map((item) => {
                let numericTotalHarga =
                  parseFloat(
                    String(item.total_harga || 0).replace(/[^0-9-]/g, ""),
                  ) || 0;
                ...
                try {
                  ...
                  if (Array.isArray(parsed)) {
                    itemNames = parsed.map(
                      (p) =>
                        `${p.nama || p.part_name || p.item || ""} (${p.qty} ${p.satuan || ""})`,
                    );
                  }
                } catch (e) {}
              ...
              if (filters.customer)
                filename += `_${filters.customer.replace(/[^a-zA-Z0-9]/g, "_")}`;
      ...
      document
        .getElementById("btn-add-po-customer")
        ?.addEventListener("click", async () => {
          ...
          if (custSel)
            custSel.innerHTML = '<option value="">Memuat data...</option>';
          ...
          if (penawaranRes.status === "success" && penawaranRes.data) {
            allApprovedPenawaran = penawaranRes.data.filter(
              (p) => p.status === "Approve" || p.status === "Approved",
            );
            if (custSel)
            ...
            const uniqueCustomers = [
              ...new Set(
                allApprovedPenawaran.map((p) => p.customer).filter(Boolean),
              ),
            ];
            uniqueCustomers.forEach((c) => {
              const opt = document.createElement("option");
          ...
          if (custSel) {
            new TomSelect("#poc_customer_select", {
              ...
              onChange: function (value) {
                ...
                sel.innerHTML = '<option value="">Pilih Penawaran...</option>';
                const filteredPenawaran = allApprovedPenawaran.filter(
                  (p) => p.customer === value,
                );
                ...
                if (
                  window._pendingAutoFillPO &&
                  window._pendingAutoFillPO.no_penawaran
                ) {
                  setTimeout(() => {
                    if (sel.tomselect) {
            ...
            if (
              window._pendingAutoFillPO &&
              window._pendingAutoFillPO.customer
            ) {
              setTimeout(() => {
      ...
      document
        .getElementById("poc_no_penawaran")
        ?.addEventListener("change", (e) => {
          ...
          if (sel.tomselect) {
            ...
            for (let i = 0; i < originalSelect.options.length; i++) {
              if (originalSelect.options[i].value === val)
      ...
      function renderPOCustomerItems(items) {
        ...
        items.forEach((item, index) => {
          ...
          let optionsHtml =
            '<option value="">-- Pilih Barang dari BOM --</option>';
          let found = false;
      ...
      window.updatePOCItemPrice = function (selectEl) {
        ...
        if (option && option.hasAttribute("data-harga")) {
          ...
          if (hg > 0) {
            row.querySelector(".poc-item-harga").value =
              window.formatRupiah(hg);
            calculatePOCTotal();
          }
        }
      };
      ...
      window.calculatePOCTotal = function () {
        ...
        rows.forEach((row) => {
          const harga = window.parseRupiah(
            row.querySelector(".poc-item-harga").value,
          );
          const qty = parseFloat(row.querySelector(".poc-item-qty").value) || 0;
          const lineSubtotal = harga * qty;
          row.querySelector(".poc-item-subtotal").value =
            window.formatRupiah(lineSubtotal);
          subtotal += lineSubtotal;
        });

        const ppnPercent =
          parseFloat(document.getElementById("poc_ppn").value) || 0;
        ...
        document.getElementById("poc_subtotal_harga_display").innerText =
          window.formatRupiah(subtotal);
        document.getElementById("poc_total_harga").value = totalHarga;
        document.getElementById("poc_total_harga_display").innerText =
          window.formatRupiah(totalHarga);
      };

      document
        .getElementById("btn-close-po-customer")
        ?.addEventListener("click", () => {
          document
            .getElementById("po-customer-modal")
            .classList.remove("active");
        });

      document
        .getElementById("po-customer-form")
        ?.addEventListener("submit", async (e) => {
          e.preventDefault();
          const btn = e.target.querySelector('button[type="submit"]');
          ...
          const payload = {
            ...
            dp_po_customer: window.parseRupiah(
              document.getElementById("poc_dp").value,
            ),
      ...
      window.openDetailPOCustomer = async function (item) {
        ...
        if (printCust)
          printCust.textContent = item.nama_customer || item.customer || "-";
        ...
        if (printPenawaran)
          printPenawaran.textContent = item.no_penawaran || "-";
        ...
        if (printTotal)
          printTotal.textContent = window.formatRupiah
            ? window.formatRupiah(item.total_harga || 0)
        ...
        const shortageTbody = document.getElementById(
          "detail-poc-shortage-tbody",
        );
        if (shortageTbody) {
          shortageTbody.innerHTML =
            '<tr><td colspan="4" style="text-align: center;"><i class="fa-solid fa-spinner fa-spin"></i> Menghitung kebutuhan bahan baku...</td></tr>';
        }
        ...
        try {
          const [resBom, resStock, resInv, resProduksi, resSJ, resInvoices] =
            await Promise.all([
              window.ERPAPI.request("get_bom"),
              window.ERPAPI.request("get_stock"),
          ...
          const relatedSPK = spkList.filter(
            (s) =>
              s.referensi_po === item.no_penawaran ||
              s.referensi_po === item.id_po_customer ||
              s.no_penawaran === item.no_penawaran ||
              s.no_penawaran === item.id_po_customer ||
              s.kode_po_customer === item.id_po_customer,
          );
          const relatedSJ = sjList.filter((s) =>
            [s.no_penawaran, s.id_po_customer, s.referensi_penawaran].some(
              (val) =>
                val &&
                (val === item.no_penawaran || val === item.id_po_customer),
            ),
          );
          const relatedInv = invList.filter((s) =>
            [s.no_penawaran, s.id_po_customer, s.referensi_penawaran].some(
          ...
          if (Array.isArray(rincian)) {
            rincian.forEach((poItem) => {
              const itemName = String(
                poItem.nama || poItem.nama_barang || poItem.part_name,
              )
                .trim()
                .toLowerCase();
              ...
              const fgMatch = inventory.find((b) => {
                const bName = String(
                  b.nama_barang || b.nama || b.nama_material || "",
                )
                  .trim()
                  .toLowerCase();
                return (
              ...
              if (fgMatch) {
                fgStock =
                  parseFloat(
                    String(fgMatch.stok || fgMatch.qty || 0).replace(
                      /[^0-9-]/g,
                      "",
                    ),
                  ) || 0;
              ...
              relatedSJ.forEach((sj) => {
                if (sj.status !== "Batal") {
                  try {
                    const itemsArr =
                      typeof sj.items === "string"
                        ? JSON.parse(sj.items)
                        : sj.items || [];
                    itemsArr.forEach((sjItem) => {
                      const sjItemName = String(
                        sjItem.nama || sjItem.item || "",
                      )
                        .trim()
                        .toLowerCase();
              ...
              relatedSPK.forEach((spk) => {
                if (spk.status !== "Batal") {
                  const spkName = String(
                    spk.kode_barang_jadi ||
                      spk.kode_barang ||
                      spk.barang_jadi ||
                      "",
                  )
                  ...
                  if (
                    spkName === itemName ||
                    itemName.includes(spkName) ||
                    spkName.includes(itemName) ||
                    spkName === String(poItem.part_number).trim().toLowerCase()
                  ) {
                  ...
                  mats.forEach((mat) => {
                    ...
                    if (!totalShortageData[matName]) {
                      const stockItem = stocks.find(
                        (s) =>
                          (s.nama || s.nama_material || "")
                            .trim()
                            .toLowerCase() === matName.toLowerCase(),
                      );
                      const currentStock = stockItem
                        ? parseFloat(
                            String(stockItem.stok)
                              .replace(/[^\d.,]/g, "")
                              .replace(/\./g, "")
                              .replace(",", "."),
                          ) || 0
                      ...
                      totalShortageData[matName] = {
                        ...
                        stock: currentStock,
                        satuan:
                          mat.satuan || (stockItem ? stockItem.satuan : "pcs"),
                      };
                    }
          ...
          if (shortageTbody) {
            const container = document.getElementById(
              "detail-poc-shortage-container",
            );
            shortageTbody.innerHTML = "";

            // Hapus material yang required-nya 0
            Object.keys(totalShortageData).forEach((k) => {
              if (totalShortageData[k].required <= 0)
                delete totalShortageData[k];
            });
            ...
            matKeys.forEach((k) => {
              if (totalShortageData[k].required > totalShortageData[k].stock)
                hasRealShortage = true;
            });
          ...
          if (itemsTbody) {
            ...
            if (Array.isArray(rincian) && rincian.length > 0) {
              rincian.forEach((poItem) => {
                const itemName =
                  poItem.nama || poItem.part_name || poItem.nama_barang || "-";
                    ...
                    try {
                      ...
                      itemsArr.forEach((sjItem) => {
                        const sjItemName = String(
                          ...
                          .trim()
                          .toLowerCase();
                        const poItemName = String(itemName)
                          .trim()
                          .toLowerCase();
                        if (
                          sjItemName === poItemName ||
                ...
                relatedSPK.forEach((spk) => {
                  ...
                  let foundName = null;
                  if (cBom && cBom.data) {
                    const match = cBom.data.find(
                      (b) => b.kode_barang === spkName || b.kode === spkName,
                    );
                    if (match) foundName = match.nama_barang || match.nama;
                  }
                  if (!foundName && cInv && cInv.data) {
                    const match = cInv.data.find(
                      (b) =>
                        b.kode_barang === spkName ||
                        b.kode === spkName ||
                        b.kode_material === spkName,
                    );
                    if (match)
                      foundName =
                        match.nama_barang || match.nama_material || match.nama;
                  }
                  if (foundName) spkNameResolved = foundName;

                  const spkNameCompare = String(spkNameResolved)
                    .trim()
                    .toLowerCase();
                  ...
                  if (
                    spk.status !== "Batal" &&
                    (spkNameCompare === itemNameCompare ||
                      itemNameCompare.includes(spkNameCompare) ||
                      spkNameCompare.includes(itemNameCompare) ||
                      spkName === poItem.part_number)
                  ) {
                ...
                if (cInvData && cInvData.data) {
                  const fgMatch = cInvData.data.find((b) => {
                    ...
                    return (
                      bName === iName ||
                      bName.includes(iName) ||
                      iName.includes(bName)
                    );
                  });
                  if (fgMatch) {
                    fgStock =
                ...
                if (sisaKekurangan === 0)
                  sisaHtml = `<span style="color: var(--success);"><i class="fa-solid fa-check"></i> Aman</span>`;
                else if (sisaKekurangan < orderedQty)
                  sisaHtml = `<span style="color: var(--warning);">${sisaKekurangan}</span>`;
                else
          ...
          if (relatedSPK.length === 0) {
            if (spkStatusEl)
              spkStatusEl.innerHTML = `<span class="badge badge-secondary" style="background: rgba(255,255,255,0.1); color: var(--text-muted);">Belum Ada SPK</span>`;
            if (spkDescEl) spkDescEl.innerText = "-";
          } else {
            const isProses = relatedSPK.some((s) => s.status !== "Selesai");
            if (isProses) {
              if (spkStatusEl)
                spkStatusEl.innerHTML = `<span class="badge badge-warning" style="background: var(--warning); color: #000;">Dalam Proses</span>`;
            } else {
              if (spkStatusEl)
                spkStatusEl.innerHTML = `<span class="badge badge-success">Selesai</span>`;
            }
            if (spkDescEl) {
              ...
              relatedSPK.forEach((s) => {
                ...
                const kode =
                  s.kode_barang_jadi || s.kode_barang || s.barang_jadi || "";
                ...
                if (cBom && cBom.data) {
                  const match = cBom.data.find(
                    (b) => b.kode_barang === kode || b.kode === kode,
                  );
                  if (match) foundName = match.nama_barang || match.nama;
                }
                if (!foundName && cInv && cInv.data) {
                ...
                let textColor = "#fff";
                if (st === "Dalam Proses" || st === "Dijadwalkan")
                  badgeColor = "var(--info)";
                if (st === "Menunggu Pengambilan") {
          ...
          if (relatedSJ.length === 0) {
            if (sjStatusEl)
              sjStatusEl.innerHTML = `<span class="badge badge-secondary" style="background: rgba(255,255,255,0.1); color: var(--text-muted);">Belum Ada DO</span>`;
            ...
            const isSjSelesai =
              relatedSJ.length > 0 &&
              relatedSJ.every((s) => s.status === "Selesai (Diterima)");
            if (isFullDelivered && isSjSelesai) {
              if (sjStatusEl)
                sjStatusEl.innerHTML = `<span class="badge badge-success">Diterima Penuh</span>`;
            } else if (isSjSelesai) {
              if (sjStatusEl)
                sjStatusEl.innerHTML = `<span class="badge badge-warning" style="background: var(--info); color: #fff;">Diterima Sebagian</span>`;
            } else {
              if (sjStatusEl)
                sjStatusEl.innerHTML = `<span class="badge badge-warning" style="background: var(--warning); color: #000;">Sedang Dikirim</span>`;
            }
            if (sjDescEl) {
              ...
              relatedSJ.forEach((s) => {
                ...
                else if (st === "Proses Pengiriman" || st === "Dikirim") {
                  badgeColor = "var(--info)";
                  textColor = "#fff";
                ...
                try {
                  ...
                  if (itemsArr.length > 0) {
                    itemHtml = itemsArr
                      .map(
                        (it) =>
                          `<div style="margin-bottom:2px;">• ${it.nama || it.item} : <b>${it.qty}</b></div>`,
                      )
                      .join("");
                  } else if (s.item) {
                    itemHtml = s.item;
          ...
          if (relatedInv.length === 0) {
            if (invStatusEl)
              invStatusEl.innerHTML = `<span class="badge badge-secondary" style="background: rgba(255,255,255,0.1); color: var(--text-muted);">Belum Ditagih</span>`;
            ...
            const totalTerbayar = relatedInv.reduce((sum, s) => {
              if (s.status_pembayaran === "Lunas")
                return (
                  sum +
                  (parseFloat(s.grand_total) ||
                    parseFloat(s.total_tagihan) ||
                    0)
                );
              if (s.status_pembayaran === "Sebagian")
                return sum + (parseFloat(s.terbayar) || 0);
              return sum;
            }, 0);
            ...
            if (isPaid) {
              if (invStatusEl)
                invStatusEl.innerHTML = `<span class="badge badge-success">Lunas</span>`;
            } else if (isPartial) {
              if (invStatusEl)
                invStatusEl.innerHTML = `<span class="badge badge-warning" style="background: var(--warning); color: #000;">Dibayar Sebagian</span>`;
            } else {
              if (invStatusEl)
                invStatusEl.innerHTML = `<span class="badge badge-danger">Belum Lunas</span>`;
            }
            if (invDescEl) {
              ...
              relatedInv.forEach((s) => {
                ...
                if (st === "Lunas") badgeColor = "var(--success)";
                  ...
                  badgeColor = "var(--warning)";
                  textColor = "#000";
                } else if (st === "Belum Lunas" || st === "Belum Bayar") {
          ...
          if (document.getElementById("track-spk-status"))
            document.getElementById("track-spk-status").innerHTML = errBadge;
          if (document.getElementById("track-sj-status"))
            document.getElementById("track-sj-status").innerHTML = errBadge;
          if (document.getElementById("track-inv-status"))
            document.getElementById("track-inv-status").innerHTML = errBadge;
        }
      ...
      document
        .getElementById("btn-close-detail-poc")
        ?.addEventListener("click", () => {
          document
            .getElementById("detail-poc-modal")
            .classList.remove("active");
        });

      // Quick Action Listeners for PO Customer Detail
      document
        .getElementById("btn-detail-act-spk")
        ?.addEventListener("click", () => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              const poRef =
                window.currentDetailPOC.id_po_customer ||
                window.currentDetailPOC.no_penawaran ||
                "";
              document.getElementById("spk_no_penawaran").value = poRef;
              document.getElementById("produksi-modal-title").textContent =
                "SPK Produksi (Referensi PO: " + poRef + ")";
              ...
              try {
                const items =
                  typeof window.currentDetailPOC.item_po === "string"
                    ? JSON.parse(window.currentDetailPOC.item_po)
                    : window.currentDetailPOC.item_po || [];
                if (items.length > 0) {
                  const iname = String(
                    items[0].nama || items[0].part_name || "",
                  ).trim();
                  ...
                  const checkInterval = setInterval(() => {
                    ...
                    if (select && select.options.length > 1) {
                      ...
                      const match = opts.find((o) =>
                        o.text.toLowerCase().includes(iname.toLowerCase()),
                      );
                      if (match) {
                        if (select.tomselect) {
      ...
      document
        .getElementById("btn-detail-act-po")
        ?.addEventListener("click", async () => {
          if (!window.currentDetailPOC) return;

          if (window.currentShortageData) {
            const keys = Object.keys(window.currentShortageData);
            const shortages = keys.filter(
              (k) =>
                window.currentShortageData[k].required >
                window.currentShortageData[k].stock,
            );
            if (shortages.length === 0) {
              const proceed = await window.showConfirm({
                title: "Stok Aman",
                message:
                  "Stok semua bahan baku sudah aman. Tetap ingin membuat pengajuan belanja manual?",
            ...
            setTimeout(() => {
              ...
              if (ref) {
                ...
                if (pocId) {
                  ...
                  let exists = Array.from(ref.options).some(
                    (opt) => opt.value === pocId,
                  );
                  if (!exists) {
                    ref.insertAdjacentHTML(
                      "beforeend",
                      `<option value="${pocId}">${pocId}</option>`,
                    );
                  }
                  ref.value = pocId;
                }
                ...
                if (shortages.length > 0) {
                  ...
                  shortages.forEach((matName) => {
                    ...
                    if (window.stockData && Array.isArray(window.stockData)) {
                      const stockItem = window.stockData.find(
                        (s) => s.nama.toLowerCase() === matName.toLowerCase(),
                      );
                      if (stockItem && stockItem.harga) {
                        const hargaNum =
                          parseInt(
                            String(stockItem.harga).replace(/\D/g, ""),
                          ) || 0;
                  ...
                  if (typeof calculatePOTotal === "function")
                    calculatePOTotal();
                }
              }
            }, 300);
      ...
      document
        .getElementById("btn-detail-act-sj")
        ?.addEventListener("click", async () => {
          if (!window.currentDetailPOC) return;
            ...
            items.forEach((it) => {
              ...
              const fg = (window.barangJadiData || []).find((b) => {
                ...
                if (nameKey) bNameRaw = String(b[nameKey]);
                  ...
                  bNameRaw = String(
                    b.nama_barang || b.nama || b.deskripsi || "",
                  );
              ...
              let rawStok = 0;
              if (fg) {
                const stokKey = Object.keys(fg).find(
                  (k) =>
                    k.includes("stok") ||
                    k.includes("qty") ||
                    k.includes("jumlah"),
              ...
              const st = fg
                ? parseInt(String(rawStok).replace(/[^0-9-]/g, "") || 0)
                : 0;
          ...
          if (isKosong) {
            const proceed = await window.showConfirm({
              ...
              message:
                "Stok Barang Jadi masih kosong atau belum mencukupi untuk pesanan ini. Tetap ingin membuat Surat Jalan?",
              confirmText: "Lanjutkan",
              cancelText: "Batal",
              type: "warning",
              icon: "⚠️",
          ...
          setTimeout(async () => {
            ...
            const poVal =
              ...
              "";
            const custVal =
              window.currentDetailPOC.nama_customer ||
              window.currentDetailPOC.customer ||
              "";
            ...
            if (tbody) {
              tbody.innerHTML =
                '<tr><td colspan="4" style="text-align:center;"><i class="fa-solid fa-spinner fa-spin"></i> Memuat Data Barang...</td></tr>';
              ...
              try {
                ...
                tbody.innerHTML =
                  '<tr><td colspan="4" style="text-align:center;">Gagal memuat rincian item</td></tr>';
              }
            }

            document
              .getElementById("surat-jalan-modal")
              .classList.add("active");
          }, 500);
        });

      document
        .getElementById("btn-detail-act-inv")
        ?.addEventListener("click", () => {
          if (!window.currentDetailPOC) return;
          document
            .getElementById("detail-poc-modal")
      ...
      document
        .getElementById("btn-detail-act-inv-dp")
        ?.addEventListener("click", () => {
          if (!window.currentDetailPOC) return;
          document
            .getElementById("detail-poc-modal")
      ...
      window.openPOCustomerModal = function (id) {
        ...
        if (pdfUrl) {
          document.getElementById("poc_file_pdf_link").innerHTML =
            `<a href="${pdfUrl}" target="_blank" style="color:var(--primary);"><i class="fa-solid fa-file-pdf"></i> Lihat File PDF Saat Ini</a>`;
        } else {
          document.getElementById("poc_file_pdf_link").innerHTML = "";
        }
        ...
        if (item.tanggal_selesai) {
          if (item.tanggal_selesai.includes("-")) {
            ...
            if (parts2.length === 3)
              tglSelesai = `${parts2[2]}-${parts2[1].padStart(2, "0")}-${parts2[0].padStart(2, "0")}`;
          }
        }
        if (document.getElementById("poc_tanggal_selesai"))
          document.getElementById("poc_tanggal_selesai").value = tglSelesai;
        document.getElementById("poc_status").value = item.status;
        document.getElementById("poc_ppn").value = item.ppn || 0;
        if (document.getElementById("poc_dp")) {
          document.getElementById("poc_dp").value = window.formatRibuan
            ? window.formatRibuan(item.dp_po_customer || 0)
            : item.dp_po_customer || 0;
        }
      ...
      window.deletePOCustomer = async function (id, btn) {
        ...
        const res = await window.ERPAPI.request("delete_po_customer", {
          id: id,
        });
        if (res.status === "success") {
          window.showToast(res.message, "success");
      ...
      document
        .getElementById("table-po-customer")
        ?.addEventListener("click", async (e) => {
          ...
          if (btnSpk) {
            ...
            document.getElementById("spk_no_penawaran").value =
              item.no_penawaran || item.id_po_customer || "";
            const selectKode = document.getElementById("spk_kode_jadi");

            if (
              cachedBOMData.length === 0 ||
              cachedInventoryData.length === 0
            ) {
              if (selectKode)
                selectKode.innerHTML =
                  '<option value="" disabled selected>Memuat data BOM...</option>';
            ...
            if (selectKode) {
              selectKode.innerHTML =
                '<option value="" disabled selected>-- Pilih Barang Jadi --</option>';
            ...
            if (selectKode) {
              ...
              let matchedBom = cachedBOMData.find(
                (b) =>
                  String(b.nama_barang).trim().toLowerCase() ===
                  String(partNames).trim().toLowerCase(),
              );
              if (matchedBom) {
                selectKode.value = matchedBom.kode_barang;
              } else {
                let existingCustom = Array.from(selectKode.options).find(
                  (o) => o.value === partNames,
                );
                if (!existingCustom) {
                  const option = document.createElement("option");
                  option.value =
                    partNames ||
                    "Custom-" + (item.no_penawaran || item.id_po_customer);
                  option.text =
                    partNames ||
                    "Dari PO " + (item.no_penawaran || item.id_po_customer);
            ...
            if (pemintaInput)
              pemintaInput.value = item.nama_customer || item.customer || "";

            if (typeof populateKruGudangDropdown === "function")
              populateKruGudangDropdown();
            document.getElementById("produksi-modal-title").textContent =
              "SPK Produksi (Referensi PO: " +
              (item.id_po_customer || item.no_penawaran) +
              ")";
              ...
              if (Array.isArray(rincian)) {
                rincian.forEach((poItem) => {
                  const matchedBom = boms.find(
                    (b) =>
                      String(b.nama_barang).trim().toLowerCase() ===
                      String(poItem.nama).trim().toLowerCase(),
                  );
                  if (matchedBom) {
                    const mats =
                      typeof matchedBom.rincian_material === "string"
                    ...
                    mats.forEach((mat) => {
                      const reqQty =
                        parseFloat(mat.qty || 0) * parseFloat(poItem.qty || 1);
                      const stockItem = stocks.find(
                        (s) =>
                          (s.nama_material || "").trim().toLowerCase() ===
                          (mat.nama_material || "").trim().toLowerCase(),
                      );
                      const currentStock = stockItem
                        ? parseFloat(
                            String(stockItem.stok).replace(/[^0-9.-]+/g, ""),
                          ) || 0
                        : 0;
                      if (reqQty > currentStock) {
                        shortageItems.push({
                          nama: stockItem
                            ? stockItem.nama_material
                            : mat.nama_material,
                          kode: stockItem
                            ? stockItem.kode_material
                            : "RM" + Math.floor(Math.random() * 10000),
            ...
            if (pemohonEl)
              pemohonEl.textContent = user.nama || user.username || "-";

            const tanggalEl = document.getElementById("po-info-tanggal");
            if (tanggalEl) {
              const now = new Date();
              tanggalEl.textContent = now.toLocaleString("id-ID", {
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
                hour: "2-digit",
            ...
            if (catEl)
              catEl.value = `Permintaan Barang Mentah (Komponen Kurang) dari PO Customer: ${item.id_po_customer} - ${item.nama_customer || ""}`;
            ...
            if (poSelect) {
              let found = Array.from(poSelect.options).find(
                (o) => o.value === item.no_penawaran,
              );
            ...
            if (tbody) {
              ...
              try {
                ...
                if (Array.isArray(rincian) && rincian.length > 0) {
                  ...
                  tbody
                    .querySelectorAll(".sj-item-qty, .sj-item-satuan")
                    .forEach((input) => {
                      input.addEventListener("input", () => {
                        const newItems = [];
                        tbody
                          .querySelectorAll(".sj-item-row")
                          .forEach((row) => {
                            newItems.push({
                              nama: row.querySelector(".sj-item-nama").value,
                              qty:
                                parseFloat(
                                  row.querySelector(".sj-item-qty").value,
                                ) || 0,
                              satuan:
                                row.querySelector(".sj-item-satuan").value ||
                                "pcs",
                            });
                          });
                        document.getElementById("sj_items_hidden").value =
                ...
                tbody.innerHTML =
                  '<tr><td colspan="4" style="text-align:center;">Gagal memuat item</td></tr>';
              }
            }
            document
              .getElementById("surat-jalan-modal")
            ...
            if (custInput)
              custInput.value = item.nama_customer || item.customer || "";
            ...
            if (tbody) {
              ...
              try {
                ...
                if (Array.isArray(rincian) && rincian.length > 0) {
                  rincian.forEach((ritem, idx) => {
                    const subtotal =
                      parseFloat(ritem.qty || 0) * parseFloat(ritem.harga || 0);
                    const tr = document.createElement("tr");
                ...
                tbody.innerHTML =
                  '<tr><td colspan="5" style="text-align:center;">Gagal memuat item</td></tr>';
              }
            }
            const ppnInput = document.getElementById("inv_ppn");
      ...
      async function loadPenawaranData(isBackgroundSync = false) {
        ...
        if (response.status === "success" && response.data) {
          ...
          if (customerSelect) {
            ...
            const prevValue = customerSelect.tomselect
            ...
            customerSelect.innerHTML =
              '<option value="">Pilih atau ketik nama Customer...</option>';
            ...
            if (custRes.status === "success" && custRes.data) {
              ...
              customers = [
                ...new Set(
                  response.data.map((item) => item.customer).filter(Boolean),
                ),
              ];
            }
          ...
          if (response.data.length === 0) {
            tbody.innerHTML =
              '<tr><td colspan="7" style="text-align: center;">Belum ada data penawaran.</td></tr>';
            return;
          }
          [...response.data].reverse().forEach((item) => {
            let badgeClass = "badge-warning";
            if (item.status === "Approved" || item.status === "Finish")
              badgeClass = "badge-success";
            ...
            if (item.status === "Approved") {
              try {
                ...
                rincianItem.forEach((rj) => {
                  const namaProduk = (rj.nama_barang || "")
                    .trim()
                    .toLowerCase();
                  ...
                  if (bomRes && bomRes.data) {
                    const bom = bomRes.data.find(
                      (b) =>
                        (b.nama_barang || "").trim().toLowerCase() ===
                        namaProduk,
                    );
                    if (bom) {
                      let rincianMaterial = [];
                      if (typeof bom.rincian_material === "string") {
                        try {
                          rincianMaterial = JSON.parse(
                            bom.rincian_material || "[]",
                          );
                        } catch (e) {}
                      ...
                      rincianMaterial.forEach((mat) => {
                        ...
                        if (namaMat) {
                          ...
                          requiredMaterials[namaMat] =
                            (requiredMaterials[namaMat] || 0) +
                            qtyMat * qtyProduk;
                        }
                      });
                    }
                ...
                if (stockRes && stockRes.data) {
                  for (const [namaMat, reqQty] of Object.entries(
                    requiredMaterials,
                  )) {
            ...
            if (item.status === "Approved") {
              actionBtns =
                `<button class="btn btn-create-po-from-penawaran" data-item='${JSON.stringify(item).replace(/'/g, "&apos;")}' style="padding: 0.4rem 0.8rem; font-size: 0.8rem; display: inline-flex; margin-right: 5px; background: var(--secondary);" title="Buat PO Customer"><i class="fa-solid fa-file-invoice"></i></button>` +
                actionBtns;
            }

            const maxNarasiLen = 50;
            const displayNarasi =
              item.narasi && item.narasi.length > maxNarasiLen
          ...
          document.querySelectorAll(".btn-edit-penawaran").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              ...
              document.getElementById("penawaran-modal-title").textContent =
                "Edit Penawaran";
              ...
              if (custSelect && custSelect.tomselect) {
                custSelect.tomselect.addOption({
                  value: item.customer,
                  text: item.customer,
                });
              ...
              document.getElementById("p_total_harga_display").textContent = (
                parseInt(
                  String(item.total_harga || "0").replace(/[^0-9]/g, ""),
                ) || 0
              ...
              document.getElementById("p_customer_address").value =
              ...
              try {
                let parsed =
                  typeof item.rincian_item === "string"
                    ? JSON.parse(item.rincian_item)
                    : item.rincian_item;
                if (Array.isArray(parsed)) items = parsed;
              } catch (e) {}
          ...
          document.querySelectorAll(".btn-delete-penawaran").forEach((btn) => {
            btn.addEventListener("click", async (e) => {
              ...
              if (ok) {
                ...
                if (typeof showToast !== "undefined")
                  showToast(
                    "Menghapus penawaran di background...",
                    "info",
                    2000,
                  );

                // Run async background sync without await blocking
                window.ERPAPI.request("delete_penawaran", {
                  id: no_penawaran,
                }).then((res) => {
                  if (res.status === "success") {
                    if (typeof showToast !== "undefined")
          ...
          document.querySelectorAll(".btn-spk-penawaran").forEach((btn) => {
            btn.addEventListener("click", async (e) => {
              ...
              if (selectKode) {
                ...
                if (matchedBom) {
                  ...
                  if (!existingCustom) {
                    ...
                    option.text =
                      partNames || "Dari Penawaran " + item.no_penawaran;
                    selectKode.add(option);
                  } else {
                    existingCustom.text =
                      partNames || "Dari Penawaran " + item.no_penawaran;
              ...
              populateKruGudangDropdown();
              document.getElementById("produksi-modal-title").textContent =
                "SPK Produksi (Referensi Penawaran: " + item.no_penawaran + ")";
              document.getElementById("produksi-modal").classList.add("active");
            });
          });
          ...
          document.querySelectorAll(".btn-po-penawaran").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              ...
              const catEl = document.getElementById("po_catatan");
              if (catEl)
                catEl.value = `Permintaan Barang Mentah (Komponen Kurang) untuk Penawaran: ${item.no_penawaran} - ${item.customer}`;
          ...
          document.querySelectorAll(".btn-print-penawaran").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              ...
              // Header settings
              document.getElementById("print_company_name").textContent =
                cachedSettings["NAMA_PERUSAHAAN"] || "NAMA PERUSAHAAN";
              document.getElementById("print_company_address").textContent =
                cachedSettings["ALAMAT"] || "Alamat Perusahaan";
              document.getElementById("print_company_phone").textContent =
                cachedSettings["NO_TELP"] || "No. Telp";

              document.getElementById("print_no").textContent =
                item.no_penawaran;
              document.getElementById("print_tanggal").textContent =
                item.tanggal
                  ? new Date(item.tanggal)
                      ...
                      .replace(/ /g, "-")
                  : "-";
              document.getElementById("print_to_customer").textContent =
                item.customer;
              ...
              if (window.ERPAPI.getCached("get_customers")) {
                ...
                const cMatch = custData.find(
                  (c) =>
                    String(c.nama_customer || c.nama || "").toLowerCase() ===
                    String(item.customer).toLowerCase(),
                );
                if (cMatch)
                  custAddress =
                    cMatch.alamat_keterangan ||
                    cMatch["alamat_/_keterangan"] ||
                    cMatch.alamat ||
                    "-";
              }
              document.getElementById("print_to_address").textContent =
                custAddress;

              document.getElementById("print_to_attn").textContent =
                info.attn || "-";
              document.getElementById("print_rev_date").textContent =
                info.rev_date || "-";
              document.getElementById("print_delivery").textContent =
                info.delivery || "-";
              document.getElementById("print_payment").textContent =
                info.payment || "-";
              ...
              document.getElementById("print_narasi").innerHTML = formatRemarks(
                item.narasi || "",
              );
              document.getElementById("print_ttd_company").textContent =
                cachedSettings["NAMA_PERUSAHAAN"] || "PT Orion Karya Sejahtera";
              document.getElementById("print_ttd_customer").textContent =
                item.customer;
          ...
          document.querySelectorAll(".btn-pay-penawaran").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              ...
              document.getElementById("pay_ref_display").textContent =
                item.no_penawaran + " - " + item.customer;
          ...
          document
            .querySelectorAll(".btn-create-po-from-penawaran")
            .forEach((btn) => {
              btn.addEventListener("click", (e) => {
                const item = JSON.parse(
                  e.currentTarget.getAttribute("data-item"),
                ...
                setTimeout(() => {
                  const btnAddPO = document.getElementById(
                    "btn-add-po-customer",
                  );
                  ...
                  setTimeout(() => {
                    ...
                    const penSelect =
                      document.getElementById("poc_no_penawaran");
                    if (custSelect && custSelect.tomselect)
                      custSelect.tomselect.lock();
                    if (penSelect && penSelect.tomselect)
                      penSelect.tomselect.lock();
                  }, 200);
                }, 50);
              });
            });
      ...
      async function loadSettingsData() {
        ...
        if (res.status === "success") {
          ...
          document.getElementById("set_nama").value =
            cachedSettings["NAMA_PERUSAHAAN"] || "";
          document.getElementById("set_alamat").value =
            cachedSettings["ALAMAT"] || "";
          document.getElementById("set_telepon").value =
            cachedSettings["NO_TELP"] || "";
          document.getElementById("set_bank_name").value =
            cachedSettings["BANK_NAME"] || "";
          document.getElementById("set_bank_account").value =
            cachedSettings["BANK_ACCOUNT"] || "";
          document.getElementById("set_bank_holder").value =
            cachedSettings["BANK_HOLDER"] || "";
        }
      }
      ...
      function calculatePenawaranTotal() {
        ...
        pItemsContainer.querySelectorAll(".p-item-row").forEach((row) => {
          const qty =
            parseInt(
              String(row.querySelector(".pi-moq").value).replace(/\D/g, ""),
            ) || 0;
          const harga =
            parseInt(
              String(row.querySelector(".pi-price").value).replace(/\D/g, ""),
            ) || 0;
      ...
      function addPenawaranItemRow(itemData = {}) {
        ...
        let optionsHtml =
          '<option value="">Ketik Nama Produk / Item...</option>';
        let found = false;
        ...
        div
          .querySelector(".pi-moq")
          .addEventListener("input", calculatePenawaranTotal);
        div
      ...
      document
        .getElementById("btn-add-p-item")
        ?.addEventListener("click", () => addPenawaranItemRow());

      document
        .getElementById("btn-add-penawaran")
        ?.addEventListener("click", async () => {
          ...
          const updateNoPenawaran = () => {
            if (
              ...
              if (
                currentVal.startsWith("OKS-") &&
                currentVal.includes("-REV00")
              ) {
                const custName = custSelect.value
                  ? custSelect.value.replace(/\s+/g, "").toUpperCase()
                  : "CUSTOMER";
                const dateStr =
                  pRevDate && pRevDate.value
                    ? pRevDate.value
...
10. Payment in Advance`;
      ...
      document
        .getElementById("btn-close-penawaran")
        ?.addEventListener("click", () => {
      ...
      penawaranForm?.addEventListener("submit", async (e) => {
        ...
        const payload = {
          ...
          total_harga:
            parseInt(
              String(document.getElementById("p_total_harga").value).replace(
                /\D/g,
                "",
              ),
            ) || 0,
        ...
        const isEdit =
          !!document.getElementById("penawaran-form").dataset.isEdit ||
          document.getElementById("penawaran-modal-title").textContent ===
            "Edit Penawaran";
        ...
        if (typeof showToast !== "undefined")
          showToast(
            "Sinkronisasi data ke server di background...",
            "info",
            3000,
          );
        ...
        if (tbody) {
          if (!isEdit) {
            ...
            const targetRow = Array.from(rows)
              .find(
                (btn) => btn.getAttribute("data-no") === payload.no_penawaran,
              )
              ?.closest("tr");
            if (targetRow) {
              ...
              if (cells.length > 6) {
                ...
                cells[4].textContent =
                  "Rp " + payload.total_harga.toLocaleString("id-ID");
                cells[6].innerHTML =
                  '<span class="badge badge-warning">Mengubah...</span>';
              }
            }
          }
        }
        ...
        window.ERPAPI.request("save_penawaran", payload).then((res) => {
          ...
          if (res.status === "success") {
            if (typeof showToast !== "undefined")
              showToast("✅ Data berhasil tersinkronisasi!", "success", 3000);
      ...
      document
        .getElementById("btn-close-payment")
        ?.addEventListener("click", () => {
      ...
      paymentForm?.addEventListener("submit", async (e) => {
        ...
        const payload = {
          ...
          total_harga:
            parseInt(
              String(document.getElementById("pay_total").value).replace(
                /\D/g,
                "",
              ),
            ) || 0,
          dp:
            parseInt(
              String(document.getElementById("pay_nominal").value).replace(
                /\D/g,
                "",
              ),
            ) || 0,
        ...
        alert(
          "Fitur Terima Pembayaran sedang diperbarui. Silakan gunakan menu Buat Invoice untuk menagih DP/Pembayaran.",
        );
        paymentModal.classList.remove("active");
        return;
      ...
      // Flow 2: GRN
      document
        .getElementById("btn-grn")
        ?.addEventListener("click", async () => {
          const kode = prompt(
            "Masukkan Kode Material yang diterima (contoh: RM001):",
          );
          const qty = prompt("Masukkan Qty yang diterima:");
          if (kode && qty) {
            const res = await window.ERPAPI.request("receive_grn", {
              no_po: "PO-Dummy",
              kode_material: kode,
              qty: qty,
            });
      ...
      async function loadBOMData(isBackgroundSync = false) {
        ...
        const render = (response) => {
          if (response.status === "success") {
            ...
            if (!response.data || response.data.length === 0) {
              tbody.innerHTML =
                '<tr><td colspan="5" style="text-align: center;">Belum ada data BOM.</td></tr>';
              return;
            }
            ...
            response.data.forEach((item, itemIdx) => {
              ...
              if (
                gambarUrl.includes("drive.google.com/uc") &&
                gambarUrl.includes("export=download")
              ) {
              ...
              tr.querySelector(".btn-detail-bom").addEventListener(
                "click",
                () => openBOMDetail(item),
              );
              if (canEdit) {
                tr.querySelector(".btn-edit-bom").addEventListener(
                  "click",
                  () => openBOMEdit(item),
                );
              }
              if (isAdmin) {
                ...
                btnDel.addEventListener("click", async (e) => {
                  ...
                  tr.style.display = "none";
                  if (typeof showToast !== "undefined")
                    showToast("Menghapus BOM...", "info");
                  window.ERPAPI.request("delete_bom", {
                    kode_barang: kode,
                  }).then((res) => {
                    if (res.status === "success") {
                      if (typeof showToast !== "undefined")
                      ...
                      if (typeof showToast !== "undefined")
                        showToast("❌ Gagal menghapus BOM", "error");
                      loadBOMData(true);
                    }
                  });
                });
        ...
        try {
          ...
          if (
            !cached ||
            JSON.stringify(cached.data) !== JSON.stringify(response.data)
          ) {
            render(response);
          }
        } catch (error) {
      ...
      function openBOMDetail(item) {
        ...
        try {
          ...
          console.error("[BOM DETAIL] Parse error rincian_proses:", e.message);
        }
        if (!Array.isArray(proses)) proses = [];
        console.log(
          "[BOM DETAIL] proses count:",
          proses.length,
          "| items:",
          JSON.stringify(
            proses.map((p, idx) => ({
              ...
              nama: p.nama,
              gambar: p.gambar
                ? "ADA (" + p.gambar.substring(0, 40) + "...)"
                : "KOSONG",
            })),
          ),
        );

        let matHtml = materials
          .map(
            (m, i) =>
              `<tr><td>${i + 1}</td><td>${m.kode || "-"}</td><td>${m.nama}</td><td>${window.parseFloatIndo(m.qty || 0).toLocaleString("id-ID", { maximumFractionDigits: 5 })}</td><td>Rp ${window.parseFloatIndo(m.harga || 0).toLocaleString("id-ID")}</td></tr>`,
          )
          .join("");
        const prosPlaceholder =
          '<span style="display:inline-flex;align-items:center;justify-content:center;width:80px;height:60px;background:rgba(255,255,255,0.05);border-radius:5px;color:var(--text-muted);font-size:0.75rem;text-align:center;line-height:1.2;">Tidak ada<br>gambar</span>';
        let prosHtml = proses
          .map((p, i) => {
        ...
        if (
          mainImgUrl.includes("drive.google.com/uc") &&
          mainImgUrl.includes("export=download")
        ) {
      ...
      function openBOMEdit(item) {
        ...
        try {
          materials =
            typeof item.rincian_material === "string"
              ? JSON.parse(item.rincian_material)
              : item.rincian_material || [];
        } catch (e) {}
        try {
          proses =
            typeof item.rincian_proses === "string"
              ? JSON.parse(item.rincian_proses)
              : item.rincian_proses || [];
        } catch (e) {}
        ...
        const updateFromBahanBaku = () => {
          ...
          if (list && val) {
            ...
            const match = options.find(
              (opt) => String(opt.value).trim().toLowerCase() === val,
            );
            if (match) {
      ...
      function addProsesRow(nama = "", gambarUrl = "") {
        ...
        div
          .querySelector(".pros-gambar-file")
          .addEventListener("change", async (e) => {
            const file = e.target.files[0];
            if (file) {
              ...
              if (file.size > 5 * 1024 * 1024) {
                showToast(
                  "❌ Ukuran file terlalu besar (max 5MB). Silakan pilih file yang lebih kecil.",
                  "error",
                  5000,
                );
                return;
              ...
              div.querySelector(".pros-gambar-base64").value =
              ...
              div.querySelector(".pros-gambar-preview img").src =
                compressedDataUrl;
              prosesImageCompressionInProgress = Math.max(
                0,
                prosesImageCompressionInProgress - 1,
              );
            }
          });
      ...
      document
        .getElementById("btn-add-proses")
        ?.addEventListener("click", () => addProsesRow());
        ...
        ?.addEventListener("change", async (e) => {
          ...
          if (file) {
            ...
            if (file.size > 5 * 1024 * 1024) {
              ...
              bomGambarBase64 = null;
              bomGambarMime = null;
              document.getElementById("bom_gambar_preview").style.display =
                "none";
              return;
            }
            ...
            document.getElementById("bom_gambar_preview").style.display =
              "block";
            document
              .getElementById("bom_gambar_preview")
              .querySelector("img").src = compressedDataUrl;
        ...
        if (poDatalist) {
          ...
          if (poData && poData.data) {
            ...
            poData.data.forEach((po) => {
              ...
              try {
                items =
                  typeof po.item_po === "string"
                    ? JSON.parse(po.item_po)
                    : po.item_po || po.items || [];
              } catch (e) {}
              items.forEach((it) => {
      ...
      bomForm?.addEventListener("submit", async (e) => {
        ...
        materialsContainer.querySelectorAll("div").forEach((div) => {
          ...
          const qty =
            window.parseFloatIndo(div.querySelector(".mat-qty")?.value) || 0;
          const harga =
            window.parseFloatIndo(div.querySelector(".mat-harga")?.value) || 0;
          if (nama)
            materials.push({
              kode: kode || "",
              nama,
        ...
        if (
          bomImageCompressionInProgress ||
          prosesImageCompressionInProgress > 0
        ) {
          showToast(
            "⏳ Tunggu sampai semua gambar selesai diproses sebelum menyimpan.",
            "warning",
            5000,
          );
          return;
        ...
        if (proses.length === 0) {
          showToast(
            "❌ Minimal ada 1 Tahapan Proses yang harus diisi",
            "error",
          );
          return;
        }
        ...
        const payload = {
          ...
          total_biaya:
            window.parseFloatIndo(totalBiayaDisplay.textContent) || 0,
        ...
        console.log("[BOM SUBMIT] Mulai submit dengan data:", {
          ...
          prosesImages: proses.map((p, i) => ({
            nama: p.nama,
            hasImage: !!p.gambar_base64,
            size: p.gambar_base64 ? p.gambar_base64.length : 0,
          })),
        ...
        if (payloadSize > 5 * 1024 * 1024) {
          showToast(
            `⚠️ Data sangat besar (${payloadSizeMB} MB). Proses mungkin memakan waktu lama. Harap tunggu...`,
            "warning",
            4000,
          );
        }
        ...
        bomModal.classList.remove("active");
        showToast(
          "Sinkronisasi data BOM ke server di background...",
          "info",
          3000,
        );
            ...
            if (targetRow) {
              ...
              if (cells.length > 3) {
                ...
                cells[3].textContent =
                  "Rp " +
                  window
                    .parseFloatIndo(payload.total_biaya)
        ...
        window.ERPAPI.request("save_bom", payload, 120000)
          .then((res) => {
            if (btnSubmit) {
            ...
            if (res.status === "success") {
              showToast(
                "✅ Data BOM berhasil disinkronisasi!",
                "success",
                3000,
              );
      ...
      async function loadProduksiData(isBackgroundSync = false) {
        ...
        try {
          ...
          if (response.status === "success" && response.data) {
            ...
            if (response.data.length === 0) {
              tbody.innerHTML =
                '<tr><td colspan="7" style="text-align: center; color: var(--text-muted);">Belum ada SPK Produksi.</td></tr>';
              return;
            }
            ...
            [...response.data].reverse().forEach((item) => {
              ...
              if (btnAmbil) {
                btnAmbil.addEventListener("click", async (e) => {
                  ...
                  if (typeof showToast !== "undefined")
                    showToast("Mengambil bahan baku...", "info");
                  const sess = JSON.parse(
                    localStorage.getItem("erp_session") || "{}",
                  );
                  window.ERPAPI.request("ambil_bahan_spk", {
                    no_spk: noSPK,
                    peminta: sess.nama_lengkap || sess.username || "",
                  }).then((res) => {
                    if (res.status === "success") {
                      if (typeof showToast !== "undefined")
                        showToast(`✅ ${noSPK} bahan baku dipotong`, "success");
                      loadProduksiData(true);
                    } else {
                      if (typeof showToast !== "undefined")
                        showToast("❌ Gagal mengambil bahan", "error");
              ...
              if (btnSelesaikan) {
                btnSelesaikan.addEventListener("click", async (e) => {
                  ...
                  btnSelesaikan.innerHTML =
                    '<i class="fa-solid fa-spinner fa-spin"></i>';
                  btnSelesaikan.disabled = true;
                  if (typeof showToast !== "undefined")
                    showToast("Menyelesaikan SPK...", "info");
                  window.ERPAPI.request("selesaikan_spk", {
                    no_spk: noSPK,
                  }).then((res) => {
                    if (res.status === "success") {
                      if (typeof showToast !== "undefined")
                        showToast(`✅ ${noSPK} selesai diproduksi`, "success");
                      loadProduksiData(true);
                    } else {
                      if (typeof showToast !== "undefined")
                        showToast("❌ Gagal menyelesaikan SPK", "error");
              ...
              if (isAdmin) {
                ...
                if (btnDel) {
                  btnDel.addEventListener("click", async (e) => {
                    ...
                    tr.style.display = "none";
                    if (typeof showToast !== "undefined")
                      showToast("Menghapus SPK...", "info");
                    window.ERPAPI.request("delete_spk", { no_spk: noSPK }).then(
                      (res) => {
                        if (res.status === "success") {
                          if (typeof showToast !== "undefined")
                            showToast(`✅ ${noSPK} dihapus`, "success");
                          loadProduksiData(true);
                        } else {
                          if (typeof showToast !== "undefined")
                            showToast("❌ Gagal menghapus SPK", "error");
                          loadProduksiData(true);
                        }
                      },
                    );
      ...
      async function populateKruGudangDropdown() {
        ...
        if (select.options.length <= 1) {
          select.innerHTML =
            '<option value="" disabled selected>Memuat data user...</option>';
          try {
            const res = await window.ERPAPI.request("get_users");
            select.innerHTML =
              '<option value="" disabled selected>Pilih Tim Produksi Terlibat</option>';
            ...
            select.innerHTML =
              '<option value="" disabled selected>Gagal memuat user</option>';
          }
        }
      }
      ...
      document
        .getElementById("btn-run-spk")
        ?.addEventListener("click", async () => {
          populateKruGudangDropdown();
          document.getElementById("produksi-form").reset();
          document.getElementById("spk-bahan-container").innerHTML =
            "Pilih barang dan isi Qty untuk melihat estimasi...";
          ...
          document.getElementById("produksi-modal-title").textContent =
            "Form SPK Produksi";
          const batchGroup = document
            .getElementById("spk_batch_count")
            ?.closest(".input-group");
          ...
          const poSelect = document.getElementById("spk_po_customer");
          select.innerHTML =
            '<option value="" disabled selected>Memuat data BOM & Inventory...</option>';
          poSelect.innerHTML =
            '<option value="" disabled selected>Memuat data PO Customer...</option>';
          ...
          poSelect.innerHTML =
            '<option value="" selected>-- Buat SPK Internal / Tanpa PO --</option>';

          if (resPO.status === "success" && resPO.data) {
            cachedPOData = resPO.data;
            resPO.data
              .filter((po) => po.status !== "Selesai" && po.status !== "Batal")
              .forEach((po) => {
                const opt = document.createElement("option");
          ...
          if (resBom.status === "success" && resBom.data) {
            ...
            select.innerHTML =
              '<option value="" disabled selected>Gagal memuat BOM</option>';
          }
        });
      ...
      async function openEditSPKModal(item) {
        ...
        let pBom = null,
          ...
          pPO = null,
          pProd = null;
        if (!cachedBOMData || cachedBOMData.length === 0)
          pBom = window.ERPAPI.request("get_bom");
        if (!cachedInventoryData || cachedInventoryData.length === 0)
          pInv = window.ERPAPI.request("get_inventory");
        if (!cachedPOData || cachedPOData.length === 0)
          pPO = window.ERPAPI.request("get_po_customer");
        if (!cachedProduksiData || cachedProduksiData.length === 0)
          pProd = window.ERPAPI.request("get_produksi");

        const results = await Promise.all([pBom, pInv, pPO, pProd]);

        if (results[0] && results[0].status === "success")
          cachedBOMData = results[0].data || [];
        if (results[1] && results[1].status === "success")
          cachedInventoryData = results[1].data || [];
        if (results[2] && results[2].status === "success")
          cachedPOData = results[2].data || [];
        if (results[3] && results[3].status === "success")
          cachedProduksiData = results[3].data || [];

        if (poSelect.tomselect) poSelect.tomselect.destroy();
        poSelect.innerHTML =
          '<option value="">-- Buat SPK Internal / Tanpa PO --</option>';
        ...
        // Determine reference
        let refPo =
          item.referensi_penawaran_x002f_po ||
          item["referensi_penawaran_/_po"] ||
          item.referensi_po ||
          item.no_penawaran ||
          item.referensi_penawaran ||
          "";
        ...
        const kodeVal =
          item.kode_barang_jadi || item.barang_jadi || item.kode_barang;
        if (kodeVal) {
          if (select.tomselect) {
        ...
        document.getElementById("spk_qty_jadi").value = parseInt(
          item.qty_produksi || item.qty || 0,
        );
      ...
      function populateSPKBarangJadi(poId) {
        ...
        if (poId) {
          const po = cachedPOData.find(
            (p) => p.id_po_customer === poId || p.no_penawaran === poId,
          );
          if (po && po.item_po) {
            try {
              const items =
                typeof po.item_po === "string"
                  ? JSON.parse(po.item_po)
                  : po.item_po;
              const poItemNames = items.map((i) =>
                String(i.nama || i.part_name || "")
                  .trim()
                  .toLowerCase(),
              );
      ...
      function updateSPKMaxQty() {
        ...
        try {
          ...
          const bomName = bom
            ? String(bom.nama_barang || "").toLowerCase()
            : "";

          const matchItem = items.find((i) => {
            const iName = String(i.nama || i.part_name || "")
              .trim()
              .toLowerCase();
            return (
              iName === bomName ||
              bomName.includes(iName) ||
              iName.includes(bomName)
            );
          });
        ...
        if (orderedQty > 0) {
          ...
          cachedProduksiData.forEach((spk) => {
            const spkRef =
              spk["kode_po_customer"] ||
              spk["referensi_penawaran"] ||
              spk["referensi_po"] ||
              spk["referensi_penawaran_/_po"] ||
              Object.values(spk)[8] ||
              "";
          ...
          if (sisaQty === 0) {
            ...
            if (
              ...
              parseInt(qtyInput.value || 0) > sisaQty ||
              parseInt(qtyInput.value || 0) === orderedQty
            ) {
      ...
      document
        .getElementById("spk_po_customer")
        ?.addEventListener("change", (e) => {
      ...
      function calculateSPKEstimasi() {
        ...
        const qty =
          parseInt(
            String(document.getElementById("spk_qty_jadi").value).replace(
              /\D/g,
              "",
            ),
          ) || 0;
        ...
        if (!bom) {
          container.innerHTML =
            '<span style="color:var(--warning);"><i class="fa-solid fa-circle-info"></i> Produk Custom atau tidak ada di BOM. Estimasi penggunaan material tidak tersedia.</span>';
          totalDisp.textContent = "0";
          btnSubmit.disabled = false;
          btnSubmit.innerHTML = "Terbitkan SPK";
          return;
        ...
        try {
          matArray =
            typeof bom.rincian_material === "string"
              ? JSON.parse(bom.rincian_material)
              : bom.rincian_material;
        } catch (e) {}
        ...
        matArray.forEach((m) => {
          ...
          const invItem = cachedInventoryData.find(
            (inv) => inv.kode_material === m.kode,
          );
          const currentStock = invItem ? parseFloat(invItem.stok) || 0 : 0;
          const shortage = reqQty - currentStock;

          const reqQtyDisp = window.formatRibuan(
            parseFloat(Number(reqQty).toFixed(5)),
          );
          const currentStockDisp = window.formatRibuan(
            parseFloat(Number(currentStock).toFixed(5)),
          );
          const shortageDisp = window.formatRibuan(
            parseFloat(Number(shortage).toFixed(5)),
          );
        ...
        totalDisp.textContent = window
          .parseFloatIndo(totalCost)
          .toLocaleString("id-ID", { maximumFractionDigits: 5 });
      ...
      document
        .getElementById("spk_qty_jadi")
        .addEventListener("input", calculateSPKEstimasi);

      document
        .getElementById("btn-close-produksi")
        ?.addEventListener("click", () => {
      ...
      produksiForm?.addEventListener("submit", async (e) => {
        ...
        const batch_count =
          parseInt(document.getElementById("spk_batch_count").value) || 1;

        const maxQty = qtyInput.getAttribute("max");
        if (maxQty !== null && parseInt(maxQty) > 0 && qty > parseInt(maxQty)) {
          window.showNotification(
            `Kuantitas tidak boleh melebihi Sisa Qty Belum SPK (${maxQty})!`,
            "error",
          );
          return;
        }
        ...
        if (batch_count > 50) {
          window.showNotification(
            `Jumlah Batch tidak masuk akal (maksimal 50 batch). Mohon periksa kembali!`,
            "error",
          );
          return;
        }
        ...
        const payload = {
          ...
          kode_po_customer:
            document.getElementById("spk_po_customer")?.value || "",
          no_penawaran:
            document.getElementById("spk_no_penawaran")?.value || "",
          peminta: document.getElementById("spk_peminta").value,
          pemberi: Array.from(
            document.getElementById("spk_pemberi").selectedOptions,
          )
            .map((o) => o.value)
            .join(", "),
        };
        ...
        produksiModal.classList.remove("active");
        if (typeof showToast !== "undefined")
          showToast("Sinkronisasi SPK ke server...", "info", 3000);
      ...
      async function loadAdminData(isBackgroundSync = false) {
        ...
        if (!isBackgroundSync)
          tbody.innerHTML =
            '<tr><td colspan="4" style="text-align: center;"><i class="fa-solid fa-spinner fa-spin"></i> Memuat data pengguna...</td></tr>';
        ...
        if (response.status === "success" && response.data) {
          ...
          document.querySelectorAll(".btn-edit-user").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              const user = JSON.parse(
                e.currentTarget.getAttribute("data-user"),
              );
          ...
          document.querySelectorAll(".btn-delete-user").forEach((btn) => {
            btn.addEventListener("click", async (e) => {
              ...
              if (ok) {
                window.ERPAPI.request("delete_user", { username }).then(
                  (res) => {
                    showToast(
                      res.status === "success"
                        ? `✅ ${res.message}`
      ...
      function openUserModal(title, data = null) {
        ...
        document.getElementById("u_nama").value = data
          ? data.nama_lengkap || data.nama || ""
          : "";
        document.getElementById("u_role").value = data
          ? data.role
          : "Staff Purchasing";
        ...
        window.ERPAPI.request("save_user", payload).then((res) => {
          if (res.status !== "success") {
            ...
            if (session.username === payload.username) {
              ...
              if (nameDisplay)
                nameDisplay.textContent = session.nama_lengkap || session.nama;
            }
          }
      ...
      document
        .getElementById("btn-close-petty-cash")
        ?.addEventListener("click", () => {
          pettyCashModal.classList.remove("active");
        });

      document
        .getElementById("petty-cash-form")
        ?.addEventListener("submit", async (e) => {
          e.preventDefault();
          ...
          if (typeof showToast !== "undefined")
            showToast("Sinkronisasi Kas ke server...", "info");
          ...
          if (res.status === "success") {
            if (typeof showToast !== "undefined")
              showToast(
                "✅ Mutasi kas berhasil " + (pcId ? "diperbarui" : "disimpan"),
                "success",
              );
            if (typeof loadFinanceKasirData === "function")
              loadFinanceKasirData();
            ...
            if (typeof loadLaporanKasData === "function")
        ...
        if (res.status === "success" && res.data) {
          if (res.data.approval_graph) {
            try {
              ...
              if (
                !allApprovalData._order ||
                !Array.isArray(allApprovalData._order)
              ) {
                allApprovalData = {
                  _order: ["Pengajuan Belanja"],
                  "Pengajuan Belanja": [],
                };
              }
            } catch (e) {
      ...
      document
        .getElementById("btn-add-kategori")
        ?.addEventListener("click", () => {
      ...
      document
        .getElementById("btn-close-kategori")
        ?.addEventListener("click", () => {
          kategoriModal.classList.remove("active");
        });

      document
        .getElementById("kategori-form")
      ...
      // Tabs
      document
        .getElementById("btn-tab-diagram")
        ?.addEventListener("click", (e) => {
          e.target.style.background = "var(--primary)";
          document.getElementById("btn-tab-flow").style.background =
            "var(--bg-glass)";
          document.getElementById("btn-tab-jabatan").style.background =
            "var(--bg-glass)";
      ...
      document
        .getElementById("btn-tab-flow")
        ?.addEventListener("click", (e) => {
          e.target.style.background = "var(--primary)";
          document.getElementById("btn-tab-diagram").style.background =
            "var(--bg-glass)";
          document.getElementById("btn-tab-jabatan").style.background =
            "var(--bg-glass)";
      ...
      document
        .getElementById("btn-tab-jabatan")
        ?.addEventListener("click", (e) => {
          e.target.style.background = "var(--primary)";
          document.getElementById("btn-tab-diagram").style.background =
            "var(--bg-glass)";
      ...
      document
        .getElementById("btn-add-jabatan")
        ?.addEventListener("click", () => {
          document.getElementById("jabatan-modal-title").textContent =
            "Tambah Jabatan";
      ...
      document
        .getElementById("btn-close-jabatan")
        ?.addEventListener("click", () => {
          jabatanModal.classList.remove("active");
        });

      document
        .getElementById("jabatan-form")
        ?.addEventListener("submit", (e) => {
          e.preventDefault();
      ...
      function renderDaftarJabatan() {
        ...
        if (daftarJabatan.length === 0) {
          listContainer.innerHTML =
            '<div style="color:var(--text-muted); font-size:0.9rem; grid-column: 1 / -1;">Belum ada daftar jabatan.</div>';
          return;
        }
        ...
        daftarJabatan.forEach((jab) => {
          ...
          btnEdit.style.cssText =
            "padding: 5px 8px; font-size: 0.75rem; background: transparent; color: var(--text-muted); box-shadow: none;";
          ...
          btnDel.style.cssText =
            "padding: 5px 8px; font-size: 0.75rem; background: transparent; color: var(--danger); box-shadow: none;";
          btnDel.onmouseover = () =>
          ...
          btnDel.onclick = async () => {
            ...
            if (ok) {
              ...
              Object.keys(allApprovalData).forEach((k) => {
                if (k === "_order") return;
                allApprovalData[k] = allApprovalData[k].filter(
                  (r) => r !== jab,
                );
              });
      ...
      function renderMasterCOA() {
        ...
        if (masterCOA.length === 0) {
          listContainer.innerHTML =
            '<div style="color:var(--text-muted); font-size:0.9rem;">Belum ada COA.</div>';
          return;
        }
      ...
      document
        .getElementById("btn-close-atasan")
        ?.addEventListener("click", () => {
          atasanModal.classList.remove("active");
        });

      document
        .getElementById("atasan-form")
        ?.addEventListener("submit", (e) => {
          e.preventDefault();
      ...
      function renderKategoriList() {
        ...
        if (allApprovalData._order.length === 0) {
          listGroup.innerHTML =
            '<div style="color:var(--text-muted); font-size:0.9rem;">Belum ada Tipe Pengajuan.</div>';
        ...
        allApprovalData._order.forEach((kat) => {
          ...
          btnDel.onclick = async () => {
            ...
            if (ok) {
              ...
              allApprovalData._order = allApprovalData._order.filter(
                (k) => k !== kat,
              );

              if (activeKategoriId === kat)
                activeKategoriId = allApprovalData._order[0];
              saveApprovalData();
              renderKategoriList();
              renderFlowBuilder();
      ...
      function renderFlowBuilder() {
        ...
        document.getElementById("activeKategoriTitle").textContent =
          activeKategoriId;
        ...
        pipeline.forEach((role, index) => {
          ...
          btnDel.style.cssText =
            "position: absolute; top: -8px; right: -8px; width: 24px; height: 24px; border-radius: 50%; background: var(--danger); color: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; box-shadow: 0 2px 5px rgba(0,0,0,0.5);";
          btnDel.onclick = () => {
        ...
        btnAdd.style.cssText =
          "background: rgba(255,255,255,0.05); border: 1px dashed var(--glass-border); box-shadow: none; display: inline-flex;";
        btnAdd.onclick = () => {
      ...
      function renderMermaidDiagram() {
        ...
        if (!allApprovalData._order || allApprovalData._order.length === 0) {
          container.innerHTML =
            '<div style="color:var(--text-muted); padding-top: 10vh;">Belum ada rute approval untuk kategori ini. Silakan atur di Tab Pengaturan Rute.</div>';
          return;
        }
        ...
        allApprovalData._order.forEach((kat, i) => {
          ...
          if (pipeline.length === 1) {
            ...
            for (let j = 0; j < pipeline.length - 1; j++) {
              ...
              if (
                pipeline[j] === sessionRole ||
                pipeline[j + 1] === sessionRole
              ) {
        ...
        if (
          !hasEdges &&
          allApprovalData._order.every(
            (k) => (allApprovalData[k] || []).length === 0,
          )
        ) {
          container.innerHTML =
            '<div style="color:var(--text-muted); padding-top: 10vh;">Rute approval kosong. Silakan atur di Tab Pengaturan Rute.</div>';
          return;
        }
        ...
        try {
          ...
          if (renderPromise && renderPromise.catch) {
            renderPromise.catch((error) => {
              ...
              container.innerHTML =
                '<div style="color:var(--danger); padding-top: 10vh;">Gagal me-render diagram. Ada kesalahan sintaks.</div>';
            });
          }
        } catch (error) {
          console.error("Mermaid render error:", error);
          container.innerHTML =
            '<div style="color:var(--danger); padding-top: 10vh;">Gagal me-render diagram.</div>';
        }
      }
      ...
      async function loadCustomerData(isBackgroundSync = false) {
        ...
        if (response.status === "success" && response.data) {
          ...
          tbody.innerHTML =
            '<tr><td colspan="6" style="text-align: center; color: var(--danger);">Gagal memuat data customer</td></tr>';
        }
      }
      ...
      function renderCustomerTable() {
        ...
        let filtered = customerData.filter((c) => {
          return (
            (c.id_customer || "").toLowerCase().includes(query) ||
        ...
        if (filtered.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="6" style="text-align: center;">Tidak ada data customer.</td></tr>';
          return;
        }
        ...
        document.querySelectorAll(".btn-edit-customer").forEach((btn) => {
          btn.addEventListener("click", (e) => {
            ...
            document.getElementById("c_nama").value =
              b.getAttribute("data-nama");
            document.getElementById("c_kontak").value =
              b.getAttribute("data-kontak") || "";
            document.getElementById("c_alamat").value =
              b.getAttribute("data-alamat");
        ...
        document.querySelectorAll(".btn-delete-customer").forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            if (ok) {
              ...
              if (typeof showToast !== "undefined")
                showToast("Menghapus Customer di background...", "info");
              ...
              window.ERPAPI.request("delete_customer", { id }).then((res) => {
                if (res.status === "success") {
                  ...
                  if (typeof showToast !== "undefined")
                    showToast(
                      "❌ Gagal: " +
                        (res.message || "Gagal menghapus customer"),
                      "danger",
                    );
      ...
      document
        ...
        ?.addEventListener("input", () => {
          if (!window.paginationStates["table-customer"])
            window.paginationStates["table-customer"] = { page: 1 };
          window.paginationStates["table-customer"].page = 1;
          renderCustomerTable();
        });
      ...
      document
        .getElementById("btn-close-customer-modal")
        ?.addEventListener("click", () => {
        ...
        ?.addEventListener("submit", async (e) => {
          ...
          if (tbody) {
            ...
            `;
              ...
              const targetRow = Array.from(rows)
                .find((btn) => btn.getAttribute("data-id") === payload.id)
                ?.closest("tr");
              if (targetRow) {
                targetRow.style.opacity = "0.5";
                const cells = targetRow.querySelectorAll("td");
          ...
          if (typeof showToast !== "undefined")
            showToast("Sinkronisasi Customer ke server...", "info");

          window.ERPAPI.request("save_customer", payload).then((res) => {
            if (res.status === "success") {
              if (typeof showToast !== "undefined")
                showToast("✅ Customer berhasil disinkronisasi!", "success");
              loadCustomerData(true);
            } else {
              if (typeof showToast !== "undefined")
                showToast(
                  "❌ Gagal: " + (res.message || "Gagal menyimpan customer"),
                  "danger",
                );
              loadCustomerData(true);
            }
      ...
      async function loadSupplierData(isBackgroundSync = false) {
        ...
        if (response.status === "success" && response.data) {
          ...
          if (tbody)
            tbody.innerHTML =
              '<tr><td colspan="6" style="text-align: center; color: var(--danger);">Gagal memuat data supplier</td></tr>';
        }
      }
      ...
      function renderSupplierTable() {
        ...
        let filtered = supplierData.filter((s) => {
          return (
            (s.id_supplier || "").toLowerCase().includes(query) ||
        ...
        if (filtered.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="6" style="text-align: center;">Tidak ada data supplier.</td></tr>';
          return;
        }
        ...
        document.querySelectorAll(".btn-delete-supplier").forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            if (confirm("Hapus supplier ini?")) {
              tr.style.display = "none";
              const res = await window.ERPAPI.request("delete_supplier", {
                id: b.dataset.id,
              });
              if (res.status === "success") {
      ...
      function populateSupplierList() {
        ...
        supplierData.forEach((s) => {
          if (s.nama_supplier) {
            ...
            option.setAttribute(
              "data-alamat",
              s.alamat ||
                s.alamat___keterangan ||
                s.alamat_keterangan ||
                s["alamat_/_keterangan"] ||
                "",
            );
        ...
        if (
          poToInput &&
          poAlamatInput &&
          !poToInput.hasAttribute("data-listener-added")
        ) {
      ...
      document
        ...
        ?.addEventListener("input", () => {
          if (!window.paginationStates["table-supplier"])
            window.paginationStates["table-supplier"] = { page: 1 };
          window.paginationStates["table-supplier"].page = 1;
          renderSupplierTable();
        });
      ...
      document
        .getElementById("btn-close-supplier-modal")
        ?.addEventListener("click", () => {
      ...
      document
        ...
        ?.addEventListener("submit", async (e) => {
          ...
          if (typeof showToast !== "undefined")
            showToast("Menyimpan Supplier...", "info");

          window.ERPAPI.request("save_supplier", payload).then((res) => {
            if (res.status === "success") {
              if (typeof showToast !== "undefined")
                showToast("✅ Supplier tersimpan!", "success");
              loadSupplierData(true);
            } else {
              if (typeof showToast !== "undefined")
                showToast("❌ Gagal: " + (res.message || "Gagal"), "danger");
              loadSupplierData(true);
            }
          });
        });
      ...
      async function openDetailPenawaran(item) {
        ...
        document.getElementById("detail-modal-title").textContent =
          "Detail Penawaran: " + (item.no_penawaran || "-");
        document.getElementById("detail_status").value =
          item.status || "Penawaran";

        const statusContainer = document.getElementById(
          "ubah-status-container",
        );
        if (statusContainer) {
          statusContainer.style.display =
            item.status === "Approved" ? "none" : "flex";
        }

        document.getElementById("detail_customer").textContent =
          item.customer || "-";
        document.getElementById("detail_tanggal").textContent =
          item.tanggal || "-";
        document.getElementById("detail_narasi").textContent =
          item.narasi || "-";

        document.getElementById("detail_total_harga").textContent =
          "Rp " +
          (
            parseInt(String(item.total_harga || "0").replace(/\D/g, "")) || 0
          ).toLocaleString("id-ID");
        ...
        // Reset UI
        const poCustomerContainer = document.getElementById(
          "detail_po_customer_container",
        );
        const poCustomerStats = document.getElementById(
          "detail_po_customer_stats",
        );

        if (poCustomerContainer && poCustomerStats) {
          poCustomerContainer.style.display = "block";
          poCustomerStats.innerHTML =
            '<p style="margin:0; color:#aaa;">Memuat data PO Customer...</p>';

          if (item.no_penawaran) {
            window.ERPAPI.request("get_po_customer")
              .then((res) => {
                if (res.status === "success" && res.data) {
                  const relatedPOs = res.data.filter(
                    (po) => po.no_penawaran === item.no_penawaran,
                  );
                  if (relatedPOs.length === 0) {
                    poCustomerStats.innerHTML =
                      '<p style="margin:0; color:#aaa;">Belum ada PO Customer untuk penawaran ini.</p>';
                  } else {
                    let html = "";
              ...
              .catch((err) => {
                poCustomerStats.innerHTML =
                  '<p style="margin:0; color:var(--danger);">Error memuat data PO Customer.</p>';
              });
          } else {
        ...
        document
          .getElementById("detail-penawaran-modal")
          .classList.add("active");
        ...
        if ((item.status || "").toLowerCase() === "approved") {
          ...
          const btnCreatePO = document.getElementById(
            "btn-detail-create-po-customer",
          );
          if (btnCreatePO) {
      ...
      document
        .getElementById("btn-update-detail-status")
        ?.addEventListener("click", async () => {
      ...
      document.addEventListener("click", (e) => {
        ...
        if (
          e.target.closest("button, .btn, input, select, textarea, a, .badge")
        )
          return;
    ...
    `;
        document
          .getElementById("customer-detail-modal")
          .classList.add("active");
      }
    ...
    `;
        document
          .getElementById("barang-jadi-detail-modal")
          .classList.add("active");
      }
    ...
    `;
      ...
      document
        .getElementById("btn-close-stock-detail")
        ?.addEventListener("click", () =>
          document
            .getElementById("stock-detail-modal")
            .classList.remove("active"),
        );
      document
        .getElementById("btn-close-customer-detail")
        ?.addEventListener("click", () =>
          document
            .getElementById("customer-detail-modal")
            .classList.remove("active"),
      ...
      document
        .getElementById("btn-close-user-detail")
        ?.addEventListener("click", () =>
          document
            .getElementById("user-detail-modal")
            .classList.remove("active"),
        );
      document
        .getElementById("btn-close-barang-jadi-detail")
        ?.addEventListener("click", () =>
          document
            .getElementById("barang-jadi-detail-modal")
            .classList.remove("active"),
      ...
      document
        .getElementById("btn-close-spk-detail")
        ?.addEventListener("click", () =>
          document
            .getElementById("spk-detail-modal")
            .classList.remove("active"),
        );
      ...
      async function loadSuratJalanData(isBackgroundSync = false) {
        ...
        if (response.status === "success" && response.data) {
          ...
          if (tbody)
            tbody.innerHTML =
              '<tr><td colspan="6" style="text-align: center; color: var(--danger);">Gagal memuat data surat jalan</td></tr>';
        }
      }
      ...
      function renderSuratJalanTable() {
        ...
        if (suratJalanData.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="6" style="text-align: center;">Tidak ada data surat jalan.</td></tr>';
          return;
        }
        ...
        sortedData.forEach((sj) => {
          ...
          if (!sjCustomer || sjCustomer === "-") {
            const poRef = poCustomerData.find(
              (p) =>
                p.id_po_customer === sjPenawaran ||
                p.no_penawaran === sjPenawaran,
            );
        ...
        window.openEditSJ = async function (idx) {
          ...
          document.getElementById("sj_tanggal").value = sjTanggal
            ? sjTanggal.split("/").reverse().join("-")
            : "";
          ...
          poRefSelect.innerHTML =
            '<option value="" disabled selected>Memuat data PO...</option>';
          custSelect.innerHTML =
            '<option value="" disabled selected>Memuat data customer...</option>';
          ...
          try {
            ...
            if (bjRes && bjRes.status === "success" && bjRes.data)
              window.barangJadiData = bjRes.data;
            if (sjRes && sjRes.status === "success" && sjRes.data)
              window.suratJalanData = sjRes.data;
              ...
              poRes.data
                .filter(
                  (po) =>
                    ...
                    po.id_po_customer === sjPenawaran ||
                    po.no_penawaran === sjPenawaran,
                )
                .forEach((po) => {
                  const opt = document.createElement("option");
                  opt.value = po.id_po_customer || po.no_penawaran;
          ...
          if (
            sjPenawaran &&
            !Array.from(poRefSelect.options).find(
              (o) => o.value === sjPenawaran,
            )
          ) {
            const o = document.createElement("option");
            o.value = sjPenawaran;
            o.textContent = sjPenawaran;
            poRefSelect.appendChild(o);
          }
          if (
            sjCustomer &&
            !Array.from(custSelect.options).find((o) => o.value === sjCustomer)
          ) {
            const o = document.createElement("option");
            o.value = sjCustomer;
            o.textContent = sjCustomer;
          ...
          new TomSelect("#sj_no_penawaran", {
            ...
            onChange: function (value) {
              if (!value) return;
              const po = loadedPOs.find(
                (p) => (p.id_po_customer || p.no_penawaran) === value,
              );
              if (po) {
                const tsCust = document.getElementById("sj_customer").tomselect;
                if (tsCust) {
                  tsCust.addOption({
                    value: po.nama_customer,
                    text: po.nama_customer,
                  });
                  ...
                  if (poItems.length === 0) {
                    ...
                    poItems.forEach((it) => {
                      ...
                      const fg = (window.barangJadiData || []).find((b) => {
                        ...
                        return (
                          bNameRaw.trim().toLowerCase() === iname.toLowerCase()
                        );
                      });
                      let rawStok = 0;
                      if (fg) {
                      ...
                      if (window.suratJalanData) {
                        window.suratJalanData.forEach((sj) => {
                          if (
                            sj.status === "Batal" ||
                            sj.no_sj === document.getElementById("sj_no").value
                          )
                            return;
                          const sjPo =
                            sj.no_penawaran ||
                            ...
                            "";
                          if (
                            sjPo === po.id_po_customer ||
                            sjPo === po.no_penawaran
                          ) {
                            let sjIt = [];
                            try {
                              sjIt =
          ...
          document.getElementById("sj_alamat_penerima").value =
            (typeof item !== "undefined" ? item.alamat_penerima : "") || "";
        ...
        document.querySelectorAll(".btn-delete-sj").forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            if (!no) {
              showToast?.(
                "Data ini rusak (tidak memiliki No Surat Jalan) dan tidak bisa dihapus dari sini. Hapus manual dari Google Sheets.",
                "error",
              );
              return;
            }
            ...
            if (
              ...
              const res = await window.ERPAPI.request("delete_surat_jalan", {
                no_sj: no,
              });
        ...
        document.querySelectorAll(".btn-finish-sj").forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            overlay.querySelector(".btn-cancel-fin").onclick = () =>
              overlay.remove();
            overlay.querySelector(".btn-confirm-fin").onclick = async () => {
              const inputPenerima = overlay
                .querySelector(".input_nama_penerima")
                .value.trim();
              if (!inputPenerima) {
              ...
              overlay.querySelector(".btn-confirm-fin").innerHTML =
                '<i class="fa-solid fa-spinner fa-spin"></i> Memproses...';
      ...
      async function printSuratJalan(item) {
        document.getElementById("print_sj_company_name").textContent =
          cachedSettings["NAMA_PERUSAHAAN"] || "PT. Orion Karya Sejahtera";
        const defaultAddress =
          "Jl. Kopo Bihbul No.45, Sayati, Kec.<br>Margahayu, Kabupaten Bandung,<br>Jawa Barat Kode pos : 40228";
        const alamatStr = cachedSettings["ALAMAT"]
          ? cachedSettings["ALAMAT"] +
            "<br>" +
            (cachedSettings["NO_TELP"] || "")
          : defaultAddress;
        document.getElementById("print_sj_company_address").innerHTML =
          alamatStr;
        ...
        if (!customerName) {
          try {
            ...
            if (poRes.status === "success" && poRes.data) {
              const poData = poRes.data.find(
                (p) =>
                  p.id_po_customer === sjPenawaran ||
                  p.no_po_customer === sjPenawaran ||
                  p.no_penawaran === sjPenawaran ||
                  p.referensi_penawaran === sjPenawaran,
              );
              if (poData)
                customerName = poData.nama_customer || poData.customer;
            }
          } catch (e) {}
        }

        document.getElementById("print_sj_customer").textContent =
          customerName || "-";
        document.getElementById("print_sj_alamat_penerima").textContent =
        ...
        document.getElementById("print_sj_plat").textContent =
          item.plat_nomor || "-";
        document.getElementById("print_sj_supir").textContent =
          item.supir || "-";
        ...
        if (elPenerima)
          elPenerima.textContent = penerimaName ? `( ${penerimaName} )` : "";
      ...
      document
        .getElementById("btn-add-surat-jalan")
        ?.addEventListener("click", async () => {
          document.getElementById("surat-jalan-form").reset();
          document.getElementById("sj_no").value = "";
          document.getElementById("sj_items_hidden").value = "";
          ...
          if (tbody)
            tbody.innerHTML =
              '<tr><td colspan="4" style="text-align:center;">Gunakan tombol dari Detail PO untuk fitur korelasi otomatis</td></tr>';
          ...
          try {
            ...
            if (poRes.status === "success" && poRes.data) {
              ...
              poRes.data
                .filter(
                  (po) => po.status !== "Selesai" && po.status !== "Batal",
                )
                .forEach((po) => {
                  const opt = document.createElement("option");
                  opt.value = po.id_po_customer || po.no_penawaran;
            ...
            poRefSelect.innerHTML =
              '<option value="" disabled>Gagal memuat PO</option>';
            custSelect.innerHTML =
              '<option value="" disabled>Gagal memuat customer</option>';
          }
      ...
      document
        .getElementById("btn-close-sj-modal")
        ?.addEventListener("click", () => {
          document
            .getElementById("surat-jalan-modal")
            .classList.remove("active");
      ...
      document
        .getElementById("surat-jalan-form")
        ?.addEventListener("submit", async (e) => {
          e.preventDefault();
          ...
          rows.forEach((tr) => {
            ...
            if (nameInput && qtyInput) {
              ...
              if (qty > maxQty && maxQty > 0) {
                ...
                alert(
                  `Peringatan: Qty pengiriman untuk ${nameInput.value} (${qty}) melebihi sisa pesanan PO (${maxQty}).`,
                );
              }
          ...
          if (items.length === 0) {
            alert(
              "Peringatan: Tidak ada item yang dikirim (Qty = 0) atau tidak ada item valid.",
            );
      ...
      async function loadInvoiceData(isBackgroundSync = false) {
        ...
        if (response.status === "success" && response.data) {
          ...
          if (tbody)
            tbody.innerHTML =
              '<tr><td colspan="6" style="text-align: center; color: var(--danger);">Gagal memuat data invoice</td></tr>';
        }
      }
      ...
      function renderInvoiceTable() {
        ...
        if (invoiceData.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="7" style="text-align: center;">Tidak ada data invoice.</td></tr>';
          return;
        }
        invoiceData.forEach((inv) => {
          ...
          else if (inv.status_pembayaran === "Batal")
            badgeClass = "badge-danger";
        ...
        document.querySelectorAll(".btn-selesai-inv").forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            const currentUser = localStorage.getItem("erp_session")
              ? JSON.parse(localStorage.getItem("erp_session")).nama
              : "Admin";
            ...
            Swal.fire({
              ...
              preConfirm: () => {
                const selectedCoa =
                  document.getElementById("selesai-inv-coa").value;
                if (!selectedCoa) {
              ...
              if (result.isConfirmed) {
                ...
                try {
                  ...
                  updatedInv.terbayar =
                    parseFloat(updatedInv.grand_total) ||
                    parseFloat(updatedInv.total_tagihan) ||
                    0;
                  updatedInv.sisa_tagihan = 0;
                  updatedInv.penyelesai = currentUser;

                  // 1. Update Invoice
                  const invRes = await window.ERPAPI.request(
                    "save_invoice",
                    updatedInv,
                  );
                  if (invRes.status === "success") {
                    showToast(
                      "Invoice diselesaikan. Mencatat mutasi kas...",
                      "info",
                    );
                    ...
                    window.ERPAPI.request("add_petty_cash", pcPayload)
                      .then((pcRes) => {
                        if (pcRes && pcRes.status === "success") {
                          showToast(
                            "Pembayaran berhasil dan Mutasi Kas tercatat otomatis",
                            "success",
                          );
                        } else {
                          Swal.fire(
                            "Mutasi Kas Gagal",
                            pcRes.message || "Gagal menyimpan mutasi kas.",
                            "warning",
                          );
                        }
                      })
                      .catch((err) => {
                        showToast(
                          "Gagal mencatat mutasi kas secara otomatis",
                          "error",
                        );
                      });
                  } else {
        ...
        window.openEditInvoice = function (idx) {
          ...
          document.getElementById("inv_tanggal").value = item.tanggal
            ? item.tanggal.split("/").reverse().join("-")
            : "";
          document.getElementById("inv_jatuh_tempo").value = item.jatuh_tempo
            ? item.jatuh_tempo.split("/").reverse().join("-")
            : "";
          const poSel = document.getElementById("inv_no_penawaran");
          if (poSel && item.no_penawaran) {
            if (
              !Array.from(poSel.options).some(
                (o) => o.value === item.no_penawaran,
              )
            ) {
              const opt = document.createElement("option");
          ...
          document.getElementById("inv_total").value = window
            .formatRupiah(item.total_tagihan || 0)
            .replace("Rp ", "");
          if (document.getElementById("inv_potongan_dp")) {
            document.getElementById("inv_potongan_dp").value = window
              .formatRupiah(item.potongan_dp || 0)
              .replace("Rp ", "");
          }
          if (document.getElementById("inv_grand_total")) {
            document.getElementById("inv_grand_total").value = window
              .formatRupiah(item.grand_total || item.total_tagihan || 0)
              .replace("Rp ", "");
          }
          if (document.getElementById("inv_terbayar"))
        ...
        document.querySelectorAll(".btn-delete-inv").forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            if (proceed) {
              if (tr) tr.style.display = "none";
              const res = await window.ERPAPI.request("delete_invoice", {
                no_invoice: no,
              });
      ...
      function printInvoice(item) {
        document.getElementById("print_inv_company_name").textContent =
          cachedSettings["NAMA_PERUSAHAAN"] || "PT. Orion Karya Sejahtera";
        const defaultAddress =
          "Jl. Kopo Bihbul No.45, Sayati, Kec.<br>Margahayu, Kabupaten Bandung,<br>Jawa Barat Kode pos : 40228";
        const alamatStr = cachedSettings["ALAMAT"]
          ? cachedSettings["ALAMAT"] +
            ...
            (cachedSettings["NO_TELP"] || "")
          : defaultAddress;
        document.getElementById("print_inv_company_address").innerHTML =
          alamatStr;

        const todayDate = new Date();
        const offset = todayDate.getTimezoneOffset() * 60000;
        const localTodayStr = new Date(todayDate.getTime() - offset)
          .toISOString()
          .split("T")[0];

        document.getElementById("print_inv_customer").textContent =
          item.customer || "-";
        ...
        document.getElementById("print_inv_alamat").textContent =
          item.alamat || alamat;

        document.getElementById("print_inv_no").textContent =
          item.no_invoice || "-";
        document.getElementById("print_inv_date").textContent =
          item.tanggal || localTodayStr;
        document.getElementById("print_inv_due").textContent =
          item.jatuh_tempo || "-";
        document.getElementById("print_inv_po").textContent =
          item.no_penawaran || item.id_po_customer || "-";
        ...
        if (item.ppn !== undefined && item.ppn !== null && item.ppn !== "") {
          ...
          const poData = window.poCustomerData.find(
            (p) =>
              p.id_po_customer === item.no_penawaran ||
              p.no_penawaran === item.no_penawaran,
          );
        ...
        if (subtotalRow)
          subtotalRow.style.display =
            ppnRate > 0 || dpAmount > 0 ? "table-row" : "none";
        if (ppnRow) ppnRow.style.display = ppnRate > 0 ? "table-row" : "none";
        if (dpRow) {
          dpRow.style.display = dpAmount > 0 ? "table-row" : "none";
        }

        document.getElementById("print_inv_subtotal").textContent =
          parseInt(subtotal).toLocaleString("id-ID");
        document.getElementById("print_inv_ppn_label").textContent =
          `PPN ${ppnRate}%`;
        document.getElementById("print_inv_ppn_amount").textContent =
          parseInt(ppnAmount).toLocaleString("id-ID");

        if (dpAmount > 0) {
          const dpPercentage = Math.round(
        ...
        document.getElementById("print_inv_total").textContent = parseInt(
          item.grand_total || item.total_tagihan || 0,
        ).toLocaleString("id-ID");
      ...
      async function printProformaInvoice(sjItem) {
        document.getElementById("print_prof_company_name").textContent =
          cachedSettings["NAMA_PERUSAHAAN"] || "PT. Orion Karya Sejahtera";
        const defaultAddress =
          "Jl. Kopo Bihbul No.45, Sayati, Kec.<br>Margahayu, Kabupaten Bandung,<br>Jawa Barat Kode pos : 40228";
        const alamatStr = cachedSettings["ALAMAT"]
          ? cachedSettings["ALAMAT"] +
            ...
            (cachedSettings["NO_TELP"] || "")
          : defaultAddress;
        document.getElementById("print_prof_company_address").innerHTML =
          alamatStr;

        const noSjStr = sjItem.no_sj || sjItem.no_surat_jalan || "";
        document.getElementById("print_prof_no").textContent =
          "PROFORMA-INV-" + noSjStr.replace("SJ-", "");
        ...
        document.getElementById("print_prof_po").textContent =
          sjItem.no_penawaran || sjItem.referensi_penawaran || "-";
        ...
        try {
          sjItems =
            typeof sjItem.items === "string"
              ? JSON.parse(sjItem.items)
              : sjItem.items || [];
        } catch (e) {}
        ...
        let poData = null;
        try {
          const searchPenawaran =
            sjItem.no_penawaran ||
            sjItem.referensi_penawaran ||
            sjItem.id_po_customer;
          ...
          if (window.poCustomerData && window.poCustomerData.length > 0) {
            poData = window.poCustomerData.find(
              (p) =>
                p.id_po_customer === searchPenawaran ||
                p.no_po_customer === searchPenawaran ||
                p.no_penawaran === searchPenawaran ||
                p.referensi_penawaran === searchPenawaran,
            );
          }
        } catch (e) {}
        ...
        const finalCustomerName =
          sjItem.customer ||
          sjItem.nama_penerima ||
          (poData ? poData.nama_customer || poData.customer : null) ||
          "-";
        document.getElementById("print_prof_customer").textContent =
          finalCustomerName;
        document.getElementById("print_prof_alamat").textContent =
          sjItem.alamat || (poData ? poData.alamat : "-") || "-";

        let poItems = [];
        if (poData) {
          try {
            poItems =
              typeof poData.item_po === "string"
                ? JSON.parse(poData.item_po)
                : poData.item_po || poData.items || [];
          } catch (e) {}
        }
        ...
        if (sjItems && sjItems.length > 0) {
          sjItems.forEach((it) => {
            ...
            const matchedPoItem = poItems.find(
              (pi) => String(pi.nama || pi.item || "").trim() === iname,
            );
            if (matchedPoItem && harga === 0) {
              harga = parseFloat(
                matchedPoItem.harga || matchedPoItem.harga_satuan || 0,
              );
            }
        ...
        document.getElementById("print_prof_subtotal").textContent =
          parseInt(total).toLocaleString("id-ID");
        document.getElementById("print_prof_ppn_label").textContent =
          `PPN ${ppnRate}%`;
        document.getElementById("print_prof_ppn_amount").textContent =
          parseInt(totalPPN).toLocaleString("id-ID");
        document.getElementById("print_prof_total").textContent =
          parseInt(grandTotal).toLocaleString("id-ID");

        const bankName = cachedSettings["BANK_NAME"] || "Bank Central Asia";
        const bankAccount = cachedSettings["BANK_ACCOUNT"] || "8106.222.423";
        const bankHolder =
          cachedSettings["BANK_HOLDER"] || "Orion Karya Sejahtera, PT";

        document.getElementById("print_prof_bank_name").textContent = bankName;
        document.getElementById("print_prof_bank_acc").textContent =
          bankAccount;
        document.getElementById("print_prof_bank_holder").textContent =
          bankHolder;
      ...
      function calculateInvoiceTotal() {
        ...
        if (document.getElementById("inv_subtotal")) {
          document.getElementById("inv_subtotal").value = window
    ...
    `;
      ...
      document
        .getElementById("btn-tarik-multi-sj")
        ?.addEventListener("click", async () => {
          const sjSelect = document.getElementById("inv_no_sj");
          let selectedSJs = [];
          ...
          if (!selectedSJs || selectedSJs.length === 0) {
            showToast?.(
              "Pilih setidaknya satu Surat Jalan terlebih dahulu!",
              "warning",
            );
            return;
          }
          ...
          try {
            // Fetch Surat Jalan
            const resSj = await window.ERPAPI.request("get_surat_jalan");
            if (resSj.status !== "success" || !resSj.data)
            ...
            if (relatedSj.length === 0)
              throw new Error(`Tidak ada data Surat Jalan yang ditemukan.`);
            ...
            if (!poNo) {
              poNo =
                relatedSj[0].no_penawaran ||
                relatedSj[0].referensi_penawaran ||
                relatedSj[0].id_po_customer ||
                "";
            ...
            if (!window.poCustomerData || window.poCustomerData.length === 0) {
              ...
              if (resPo.status === "success")
                window.poCustomerData = resPo.data;
            }
            let poData = null;
            if (
              poNo &&
              window.poCustomerData &&
              window.poCustomerData.length > 0
            ) {
              poData = window.poCustomerData.find(
                (p) =>
                  String(p.id_po_customer).trim().toLowerCase() ===
                    poNo.toLowerCase() ||
                  String(p.no_po_customer).trim().toLowerCase() ===
                    poNo.toLowerCase() ||
                  String(p.no_penawaran).trim().toLowerCase() ===
                    poNo.toLowerCase(),
              );
            }
            ...
            if (!document.getElementById("inv_customer").value) {
              document.getElementById("inv_customer").value =
                relatedSj[0].customer ||
                relatedSj[0].nama_penerima ||
                (poData ? poData.nama_customer || poData.customer : "");
            }
            ...
            Object.values(combinedItems).forEach((it) => {
              ...
              const matchedPoItem = poItems.find(
                (pi) => String(pi.nama || pi.item || "").trim() === it.nama,
              );
              if (matchedPoItem && harga === 0) {
                harga = parseFloat(
                  matchedPoItem.harga || matchedPoItem.harga_satuan || 0,
            ...
            if (poNo && window.invoiceData) {
              ...
              window.invoiceData
                .filter((inv) => String(inv.no_penawaran).trim() === poNo)
                .forEach((inv) => {
                  let isDpInv = false;
                  let dpItemValue = 0;
                  try {
                    ...
                    items.forEach((it) => {
                      ...
                      if (
                        ...
                        dpItemValue +=
                          parseFloat(
                            it.harga || it.harga_satuan || it.price || 0,
                          ) * parseFloat(it.qty || 1);
                      }
                    });
                  } catch (e) {}
              ...
              if (sisaDp > 0) {
                let currentSub = 0;
                document
                  .querySelectorAll("#inv-items-tbody tr")
                  .forEach((tr) => {
                    const qI = tr.querySelector(".inv-item-qty");
                    const pI = tr.querySelector(".inv-item-price");
                    if (qI && pI)
                      currentSub +=
                        parseFloat(qI.value || 0) *
                        parseFloat(
                          String(pI.value).replace(/[^0-9]/g, "") || 0,
                        );
                  });
            ...
            showToast?.(
              `Berhasil menarik ${Object.keys(combinedItems).length} item dari ${relatedSj.length} Surat Jalan!`,
              "success",
            );
          } catch (error) {
      ...
      document
        .getElementById("btn-add-inv-item")
        ?.addEventListener("click", () => {
          addInvoiceItemRow();
        });
      ...
      document
        ...
        ?.addEventListener("click", async () => {
          ...
          document.getElementById("inv_tanggal").value = new Date(
            ...
            .split("T")[0];
          document.getElementById("inv_jatuh_tempo").value = new Date(
            today.getTime() - offset + 30 * 24 * 60 * 60 * 1000,
          )
            .toISOString()
            .split("T")[0]; // +30 days
          ...
          if (sjSelect) {
            ...
            sjSelect.innerHTML =
              '<option value="" disabled>Memuat Surat Jalan...</option>';
          }
            ...
            try {
              ...
              if (res.status === "success" && res.data) {
                ...
                res.data
                  .sort(
                    (a, b) =>
                      new Date(b.tanggal_kirim || b.tanggal || 0) -
                      new Date(a.tanggal_kirim || a.tanggal || 0),
                  )
                  .forEach((sj) => {
                    const sjNo = String(
                      sj.no_sj || sj.no_surat_jalan || "",
                    ).trim();
            ...
            try {
              ...
              if (poRes.status === "success" && poRes.data) {
                ...
                poRes.data
                  ...
                  .forEach((p) => {
                    const poId =
                      p.id_po_customer || p.no_po_customer || p.no_penawaran;
                    if (poId) {
                      const option = document.createElement("option");
                      option.value = poId;
      ...
      document
        ...
        ?.addEventListener("change", async (e) => {
          ...
          if (sjSelect)
            sjSelect.innerHTML =
              '<option value="">Memuat Surat Jalan...</option>';
          ...
          if (window.allSjData) {
            ...
            const sortedSj = [...window.allSjData].sort(
              (a, b) =>
                new Date(b.tanggal_kirim || b.tanggal || 0) -
                new Date(a.tanggal_kirim || a.tanggal || 0),
            );
          ...
          if (selectedPo && window.invoiceData) {
            ...
            if (window.poCustomerData) {
              selectedPoData = window.poCustomerData.find(
                (p) =>
                  String(p.id_po_customer).trim() === selectedPo ||
                  String(p.no_penawaran).trim() === selectedPo,
              );
            }

            let totalDPTerbayar = 0;
            if (selectedPoData) {
              const dpKey = Object.keys(selectedPoData).find((k) =>
                k.toLowerCase().includes("dp"),
              );
              if (dpKey && selectedPoData[dpKey]) {
                totalDPTerbayar =
                  window.parseRupiah(selectedPoData[dpKey]) || 0;
              }
            }
            ...
            window.invoiceData.forEach((inv) => {
              ...
              if (
                sPo === selectedPo &&
                inv.no_invoice !== currentInvNo &&
                inv.status_pembayaran !== "Batal"
              ) {
            ...
            let th = 0;
            if (selectedPoData) {
              const thKey = Object.keys(selectedPoData).find(
                (k) =>
                  k.toLowerCase().includes("total_harga") ||
            ...
            if (invPotonganDp) {
              ...
              if (dpRatio > 0) {
                ...
                invPotonganDp.title =
                  "DP dihitung otomatis berdasarkan persentase";
              } else {
      ...
      document
        .getElementById("btn-close-inv-modal")
        ?.addEventListener("click", () => {
      ...
      document
        ...
        ?.addEventListener("submit", async (e) => {
          ...
          const payload = {
            ...
            total_tagihan: parseFloat(
              String(document.getElementById("inv_total").value).replace(
                /[^0-9]/g,
                "",
              ) || 0,
            ),
            potongan_dp: document.getElementById("inv_potongan_dp")
              ? parseFloat(
                  String(
                    document.getElementById("inv_potongan_dp").value,
                  ).replace(/[^0-9]/g, "") || 0,
                )
              : 0,
            grand_total: document.getElementById("inv_grand_total")
              ? parseFloat(
                  String(
                    document.getElementById("inv_grand_total").value,
                  ).replace(/[^0-9]/g, "") || 0,
                )
              : 0,
            terbayar: 0,
            ...
            ppn: document.getElementById("inv_ppn")
              ? document.getElementById("inv_ppn").value
              : 0,
            items: items,
            pembuat:
          ...
          if (typeof window.invoiceData !== "undefined") {
            const isEdit = window.invoiceData.findIndex(
              (inv) => inv.no_invoice === payload.no_invoice,
            );
            if (isEdit > -1) {
          ...
          if (cachedStr) {
            try {
              const c = JSON.parse(cachedStr);
              if (c && c.data && Array.isArray(c.data.data)) {
                const existingIdx = c.data.data.findIndex(
                  (inv) => inv.no_invoice === payload.no_invoice,
                );
                if (existingIdx > -1) {
    ...
    async function loadCOAData(isBackgroundSync = false) {
      try {
        ...
        if (result.status === "success") {
          ...
          document.getElementById("table-coa").innerHTML =
            '<tr><td colspan="3" style="text-align:center; color:red;">Gagal memuat data: ' +
            result.message +
            "</td></tr>";
        }
      } catch (e) {
    ...
    function isChildCOA(parentKode, childKode) {
      ...
      return (
        childKode.startsWith(parentKode + ".") &&
        getCOADepth(childKode) === getCOADepth(parentKode) + 1
      );
    }
    ...
    function renderCOATree(data) {
      ...
      if (data.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="3" style="text-align: center;">Tidak ada data COA</td></tr>';
        return;
      }
      ...
      const renderNode = (node, depth, parentKode) => {
        ...
        if (hasChildren) {
          node.children.forEach((child) =>
            renderNode(child, depth + 1, node.kode),
          );
        }
      };
    ...
    window.toggleCOAChildren = function (parentKode) {
      ...
      const icon = document.getElementById(
        "coa-icon-" + parentKode.replace(/[^a-zA-Z0-9]/g, "-"),
      );
    ...
    function hideAllDescendants(parentKode) {
      const rows = document
        .getElementById("table-coa")
        .querySelectorAll(".coa-row");
      const icon = document.getElementById(
        "coa-icon-" + parentKode.replace(/[^a-zA-Z0-9]/g, "-"),
      );
      if (icon) {
    ...
    function populateCOADatalist(data) {
      ...
      if (parentSelect) {
        parentSelect.innerHTML =
          '<option value="">-- Buat Root Baru / Ketik Manual --</option>';
        data.forEach((item) => {
          const opt = document.createElement("option");
          opt.value = item.kode;
    ...
    function renderPettyCashCOATree(data) {
      ...
      if (data.length === 0) {
        tbody.innerHTML =
          '<tr><td style="text-align: center; padding: 10px;">Tidak ada data COA</td></tr>';
        return;
      }
    ...
    window.togglePCCoaChildren = function (e, parentKode) {
      ...
      const icon = document.getElementById(
        "pc-coa-icon-" + parentKode.replace(/[^a-zA-Z0-9]/g, "-"),
      );
    ...
    window.deleteCOA = async function (kode) {
      if (
        !confirm(
          "Hapus COA ini? Semua sub-COA tidak akan terhapus tapi mungkin kehilangan parent!",
        )
      )
        return;
      try {
    ...
    document
      .getElementById("coa_parent_kode")
    ...
    document
      ...
      ?.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          const result = await ERPAPI.request("save_coa", {
            old_kode: oldKode,
            kode: kode,
            keterangan: name,
          });
    ...
    function renderRBACTable() {
      ...
      roles.forEach((role) => {
        ...
        MENUS.forEach((menu, index) => {
          ...
          if (!perm) {
            ...
            const isProduksi =
              role.toLowerCase().includes("produksi") ||
              role.toLowerCase().includes("gudang");
            const isFinance =
              role.toLowerCase().includes("finance") ||
              role.toLowerCase().includes("accounting");
            ...
            if (!isAdmin) {
              if (
                isPurchasing &&
                ["purchasing", "po-internal", "supplier"].includes(menu.id)
              )
                defaultView = true;
              if (
                isProduksi &&
              ...
              if (
                isFinance &&
                ["finance", "invoice", "laporan-kas", "coa"].includes(menu.id)
              )
                defaultView = true;
              if (
                isSales &&
                ["sales", "po-customer", "customer"].includes(menu.id)
              )
                defaultView = true;
            }
      ...
      document.querySelectorAll(".rbac-cb").forEach((cb) => {
        cb.addEventListener("change", (e) => {
          ...
          const perm = currentRBACData.find(
            (p) =>
              p.role.toLowerCase() === role.toLowerCase() &&
              p.menu_id === menuId,
          );
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        try {
          const response = await window.ERPAPI.request("save_permissions", {
            permissions: currentRBACData,
          });
          if (response.status === "success") {
            showToast("✅ Hak Akses berhasil disimpan!", "success");
            if (typeof window.checkSession === "function")
              window.checkSession(); // Re-sync session immediately for current user
          } else {
    ...
    window.applyRBACToButtons = function (menuId, container) {
      ...
      dom
        .querySelectorAll('[id^="btn-add"], [class*="btn-add"]')
        .forEach((btn) => {
          btn.style.display = canAdd ? "" : "none";
        });
      ...
      dom
        .querySelectorAll('[class*="btn-delete"], [class*="btn-del"]')
        .forEach((btn) => {
    ...
    async function loadGRNData(isBackgroundSync = false) {
      ...
      try {
        ...
        if (poRes.status === "success") {
          currentGRN_PO_Data = poRes.data.filter(
            (po) =>
              po.status === "Disetujui (Sedang Dibelikan)" ||
              po.status === "Parsial" ||
              po.status === "Selesai" ||
              po.status === "Selesai (Barang Diterima)",
          );
        }
    ...
    window.openGRNDetail = function (no_po) {
      const po = currentGRN_PO_Data.find(
        (p) => String(p.no_po) === String(no_po),
      );
      if (!po) return;

      // Hitung qty yang sudah diterima
      const historyPO = currentGRN_History.filter(
        (g) => String(g.no_po) === String(no_po),
      );
      let mapDiterima = {};
      let riwayatHTML = "";
      ...
      if (historyPO.length > 0) {
        ...
        historyPO.forEach((g) => {
          let itemsStr = (g.daftar_item_parsed || [])
            .map((i) => `${i.nama} (${i.qty_diterima || i.qty_terima || 0})`)
            .join(", ");
          ...
          let gambarHTML = g.gambar
            ? `<img src="${g.gambar}" style="max-height: 30px; border-radius: 4px; cursor: zoom-in;" onclick="${clickFunc}" title="Klik untuk memperbesar" />`
            : "-";
          ...
          (g.daftar_item_parsed || []).forEach((item) => {
            ...
            mapDiterima[key] += Number(
              item.qty_diterima || item.qty_terima || 0,
            );
          });
        });
    ...
    window.openGRNModal = function (no_po) {
      ...
      const session = localStorage.getItem("erp_session");
      if (session) {
        document.getElementById("grn_penerima").value =
          JSON.parse(session).nama;
      }
    ...
    document.getElementById("btn-close-grn")?.addEventListener("click", () => {
      document
        .getElementById("grn-penerimaan-modal")
        .classList.remove("active");
    });
    ...
    document
      .getElementById("grn-penerimaan-form")
      ?.addEventListener("submit", async (e) => {
        e.preventDefault();
        const btnSubmit = document.querySelector(
          '#grn-penerimaan-form button[type="submit"]',
        );
        ...
        try {
          ...
          btnSubmit.innerHTML =
            '<i class="fa-solid fa-save"></i> Simpan Penerimaan';
          btnSubmit.disabled = false;
        }
      });
    ...
    window.toggleFilterKasCOAChildren = function (e, parentKode) {
      ...
      const icon = document.getElementById(
        "filter-kas-coa-icon-" + parentKode.replace(/[^a-zA-Z0-9]/g, "-"),
      );
    ...
    window.selectFilterKasCOA = function (kode, keterangan) {
      document.getElementById("filter-kas-coa").value = kode
        ? `${kode} - ${keterangan}`
        : "";
      const display = document.getElementById("filter-kas-coa-display-text");
      if (display) {
        display.textContent = kode
          ? `${kode} - ${keterangan}`
          : "-- Semua Kategori COA --";
        display.style.color = "var(--text-main)";
      }
    ...
    document
      .getElementById("filter-kas-coa-display")
      ?.addEventListener("click", function (e) {
    ...
    function applyLaporanKasFilter() {
      ...
      if (coaFilter) {
        ...
        filteredData = filteredData.filter(
          (item) => item.coa && item.coa.startsWith(prefix),
        );
      }
    ...
    document
      .getElementById("btn-filter-kas")
      ?.addEventListener("click", applyLaporanKasFilter);
    ...
    function renderLaporanKasTable(data) {
      ...
      if (data.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="7" style="text-align: center;">Tidak ada data kas pada periode/filter tersebut.</td></tr>';
      ...
      document.getElementById("summary-kas-masuk").textContent =
        `Rp ${totalMasuk.toLocaleString("id-ID")}`;
      document.getElementById("summary-kas-keluar").textContent =
    ...
    async function loadTransaksiGudangData(isBackgroundSync = false) {
      if (!isBackgroundSync) {
        document.getElementById("transaksi-tbody").innerHTML =
          '<tr><td colspan="8" style="text-align: center;"><i class="fa-solid fa-spinner fa-spin"></i> Memuat data...</td></tr>';
      }
      ...
      try {
        ...
        if (res.status === "success" && res.data) {
          ...
          if (res.data.length === 0) {
            tbody.innerHTML =
              '<tr><td colspan="8" style="text-align: center;">Belum ada riwayat transaksi.</td></tr>';
            return;
          }
          ...
          res.data.reverse().forEach((item) => {
            ...
            const badgeClass =
              jenisVal === "IN" ? "badge-success" : "badge-danger";
            tr.innerHTML = `
            ...
            tr.addEventListener("click", (e) => {
              if (e.target.closest("button")) return;
              document.getElementById("td_id").textContent =
                item.id_transaksi || "-";
              document.getElementById("td_tanggal").textContent =
                item.tanggal || "-";
              document.getElementById("td_jenis").textContent =
                item.jenis || item.jenis_in_out || "-";
              document.getElementById("td_jenis").className =
                "badge " + badgeClass;
              document.getElementById("td_referensi").textContent =
                item.referensi || "-";
              document.getElementById("td_bahan").textContent =
                item.kode_material || "-";
              document.getElementById("td_qty").textContent = item.qty || "-";
              document.getElementById("td_pic").textContent = item.pic || "-";
              document.getElementById("td_peminta").textContent =
                item.peminta || "-";
              document.getElementById("td_pemberi").textContent =
                item.pemberi || "-";
              document.getElementById("td_keterangan").textContent =
                item.keterangan || "-";
              document
                .getElementById("transaksi-detail-modal")
                .classList.add("active");
            });
          });
          ...
          document.querySelectorAll(".btn-edit-transaksi").forEach((btn) => {
            btn.addEventListener("click", async (e) => {
              ...
              document.getElementById("trx_id_transaksi").value =
                item.id_transaksi;
              document.getElementById("trx_jenis").value =
                item.jenis || item.jenis_in_out || "OUT";
              ...
              document.getElementById("transaksi-modal-title").textContent =
                "Edit Transaksi Gudang";
              document
                .getElementById("transaksi-gudang-modal")
                .classList.add("active");
            });
          });
          ...
          document.querySelectorAll(".btn-del-transaksi").forEach((btn) => {
            btn.addEventListener("click", async (e) => {
              ...
              if (
                ...
                btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
                const res = await window.ERPAPI.request(
                  "delete_transaksi_gudang",
                  { id },
                );
                if (res.status === "success") {
          ...
          document.querySelectorAll(".btn-approve-transaksi").forEach((btn) => {
            btn.addEventListener("click", async (e) => {
              ...
              window.ERPAPI.request("approve_transaksi", {
                id_transaksi: id,
                pemberi: pemberiName,
              }).then((res) => {
                if (res.status === "success") {
                  if (typeof showToast !== "undefined")
                    showToast("✅ Transaksi disetujui", "success");
                  loadTransaksiGudangData(true);
                } else {
                  if (typeof showToast !== "undefined")
                    showToast("❌ Gagal menyetujui: " + res.message, "error");
                  loadTransaksiGudangData(true);
                }
              });
            });
    ...
    // Modal Logic
    document
      .getElementById("btn-add-transaksi")
      ?.addEventListener("click", async () => {
        // Populate dropdown
        const res = await window.ERPAPI.request("get_stock");
        const select = document.getElementById("trx_kode_material");
        ...
        document.getElementById("trx_peminta").value =
          sess.nama_lengkap || sess.username || "";

        document.getElementById("transaksi-modal-title").textContent =
          "Buat Transaksi Gudang (Bahan Baku)";
        document.getElementById("trx_id_transaksi").value = "";
    ...
    document
      .querySelector(".btn-close-transaksi")
      ?.addEventListener("click", () => {
        document
          .getElementById("transaksi-gudang-modal")
          .classList.remove("active");
    ...
    document
      .querySelector(".btn-close-transaksi-detail")
      ?.addEventListener("click", () => {
        document
          .getElementById("transaksi-detail-modal")
          .classList.remove("active");
    ...
    document
      .getElementById("trx_kode_material")
      ?.addEventListener("change", (e) => {
        const val = e.target.value;
    ...
    document
      .getElementById("transaksi-gudang-form")
      ?.addEventListener("submit", async (e) => {
        e.preventDefault();
        ...
        if (jenis === "OUT") {
          ...
          const stok = item
            ? parseFloat(String(item.stok).replace(/[^0-9.-]/g, ""))
            : 0;
          if (qty > stok && !idTrx) {
            // Allow bypass if editing (we will handle proper logic in backend)
    ...
    async function loadFinanceKasirData() {
      try {
        ...
        if (res.status === "success") {
          ...
          if (elMasuk)
            elMasuk.textContent = "Rp " + totalMasuk.toLocaleString("id-ID");
          if (elKeluar)
            elKeluar.textContent = "Rp " + totalKeluar.toLocaleString("id-ID");

          if (tbody) {
            if (todayData.length === 0) {
              tbody.innerHTML =
                '<tr><td colspan="6" style="text-align: center;">Belum ada mutasi kas hari ini.</td></tr>';
            } else {
              tbody.innerHTML = "";
              todayData.sort((a, b) => {
                const dateA = window.parseDDMMYYYY(a.waktu.split(" ")[0]);
        ...
        if (btnEdit) {
          ...
          if (displayText) {
            displayText.textContent =
              btnEdit.dataset.coa || "-- Pilih Pos Akuntansi --";
            displayText.style.color = btnEdit.dataset.coa
              ? "var(--text-main)"
              : "#999";
          }
        ...
        if (btnDelete) {
          ...
          if (typeof window.showConfirm === "function") {
            proceed = await window.showConfirm({
              ...
              message:
                'Apakah Anda yakin ingin menghapus mutasi kas ini?<br><span style="font-size:0.82rem;color:rgba(255,255,255,0.4);">Data akan dihapus permanen dan tidak bisa dikembalikan.</span>',
              confirmText: "Ya, Hapus",
              cancelText: "Batal",
            });
          } else {
            proceed = confirm(
              "Apakah Anda yakin ingin menghapus mutasi kas ini?",
            );
          }
          ...
          const res = await window.ERPAPI.request("delete_petty_cash", {
            id: pcId,
          });
          if (res.status === "success") {
